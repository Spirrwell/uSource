...
template <class T, class U> static void SetBits(T& iBitVector, U bits) { iBitVector = iBitVector | bits; }

template <class T, class U> static void ClearBits(T& iBitVector, U bits) { iBitVector = (iBitVector) & ~bits; }

template <class T, class U> static bool FBitSet(T iBitVec, U bit) { return iBitVec & bit; }
...
template <class T> T ToLittleEndian(T& x)
{
	if (sizeof(T) == 1)
		return x;
	T y = x;
...
template <class T> T ToBigEndian(T& x) { return x; }
...
template <class T> T ToLittleEndian(T& x) { return x; }

template <class T> T ToBigEndian(T& x)
{
	if (sizeof(T) == 1)
		return x;
	T y = x;
