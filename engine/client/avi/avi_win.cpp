...
static dllfunc_t msvfw_funcs[] = {
	{"DrawDibOpen", (void**)&pDrawDibOpen}, {"DrawDibDraw", (void**)&pDrawDibDraw}, {"DrawDibClose", (void**)&pDrawDibClose}, {NULL, NULL}};
...
static dllfunc_t msacm_funcs[] = {{"acmStreamOpen", (void**)&pacmStreamOpen},
...
static dllfunc_t avifile_funcs[] = {{"AVIFileExit", (void**)&pAVIFileExit},
...
qboolean AVI_ACMConvertAudio(movie_state_t* Avi)
{
	...
	pAVIStreamRead(Avi->audio_stream, Avi->cpa_blocknum * Avi->cpa_blockalign, Avi->cpa_blockalign, Avi->cpa_srcbuffer, Avi->cpa_blockalign, NULL,
		       NULL);
...
qboolean AVI_SeekPosition(movie_state_t* Avi, dword offset)
{
	...
	if (offset < Avi->cpa_blockoffset) // well, shit. we can't seek backwards... restart
	{
		...
		pAVIStreamRead(Avi->audio_stream, Avi->cpa_blocknum * Avi->cpa_blockalign, Avi->cpa_blockalign, Avi->cpa_srcbuffer,
			       Avi->cpa_blockalign, NULL, NULL);
		pacmStreamConvert(Avi->cpa_conversion_stream, &Avi->cpa_conversion_header, ACM_STREAMCONVERTF_BLOCKALIGN | ACM_STREAMCONVERTF_START);
...
int AVI_GetAudioChunk(movie_state_t* Avi, char* audiodata, int offset, int length)
{
	...
	if (Avi->audio_codec == WAVE_FORMAT_PCM)
	{
		...
		pAVIStreamRead(Avi->audio_stream, offset / Avi->audio_bytes_per_sample, length / Avi->audio_bytes_per_sample, audiodata, length,
			       &result, NULL);
		return result;
	}
	else
...
movie_state_t* AVI_GetState(int num) { return &avi[num]; }
