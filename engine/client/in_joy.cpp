...
============
*/
qboolean Joy_IsActive(void) { return joy_found->value && joy_enable->value; }
...
static void Joy_ProcessTrigger(const engineAxis_t engineAxis, short value)
{
	...
	if (joyaxis[engineAxis].val > trigThreshold && joyaxis[engineAxis].prevval <= trigThreshold) // ignore random press
	{
		Key_Event(trigButton, true);
	}
	else if (joyaxis[engineAxis].val < trigThreshold && joyaxis[engineAxis].prevval >= trigThreshold) // we're unpressing (inverted)
	{
...
static int Joy_GetHatValueForAxis(const engineAxis_t engineAxis)
{
	...
	if (joyaxis[engineAxis].val < -threshold && joyaxis[engineAxis].prevval >= -threshold) // we're unpressing (inverted)
	{
		return negative;
	}
	return 0;
...
#if !defined(XASH_SDL)
	*dpitch +=
		joy_pitch->value * (float)joyaxis[JOY_AXIS_PITCH].val / (float)SHRT_MAX * host.realframetime; // abs axis rotate is frametime related
...
void Joy_Init(void)
{
	...
	joy_side_key_threshold =
		Cvar_Get("joy_side_key_threshold", "24576", FCVAR_ARCHIVE, "side axis key event emit threshold. Value from 0 to 32767");
	joy_forward_key_threshold =
		Cvar_Get("joy_forward_key_threshold", "24576", FCVAR_ARCHIVE, "forward axis key event emit threshold. Value from 0 to 32767");
	...
	joy_axis_binding = Cvar_Get("joy_axis_binding", "sfpyrl", FCVAR_ARCHIVE,
...
===========
*/
void Joy_Shutdown(void) { Cvar_FullSet("joy_found", 0, FCVAR_READ_ONLY); }
