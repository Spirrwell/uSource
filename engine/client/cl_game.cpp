...
void CL_CenterPrint(const char* text, float y)
{
	...
	while (*s)
	{
		if (*s == '\n')
		{
			...
			width += clgame.scrInfo.charWidths[*s];
		s++;
		length++;
	}
...
void CL_DrawScreenFade(void)
{
	...
	ref.dllFuncs.R_DrawStretchPic(0, 0, refState.width, refState.height, 0, 0, 1, 1, R_GetBuiltinTexture(REF_WHITE_TEXTURE));
...
====================
*/
int CL_GetMaxClients(void) { return cl.maxclients; }
...
=============
*/
model_t* CL_LoadClientSprite(const char* filename) { return CL_LoadSpriteModel(filename, SPR_CLIENT, 0); }
...
=============
*/
static void pfnConsolePrint(const char* string) { Con_Printf("%s", string); }
...
=============
*/
static void pfnCenterPrint(const char* string) { CL_CenterPrint(string, 0.25f); }
...
=============
*/
static const char* pfnPhysInfo_ValueForKey(const char* key) { return Info_ValueForKey(cls.physinfo, key); }
...
=============
*/
static const char* pfnServerInfo_ValueForKey(const char* key) { return Info_ValueForKey(cl.serverinfo, key); }
...
=============
*/
static float pfnGetClientMaxspeed(void) { return cl.local.maxspeed; }
...
=============
*/
static int pfnIsNoClipping(void) { return (cl.frames[cl.parsecountmod].playerstate[cl.playernum].movetype == MOVETYPE_NOCLIP); }
...
=============
*/
cl_entity_t* CL_GetViewModel(void) { return &clgame.viewent; }
...
=============
*/
static float pfnGetClientTime(void) { return cl.time; }
...
=============
*/
static int pfnIsSpectateOnly(void) { return (cls.spectator != 0); }
...
=============
*/
static word pfnPrecacheEvent(int type, const char* psz) { return CL_EventIndex(psz); }
...
=============
*/
int pfnLocalPlayerDucking(void) { return (cl.local.usehull == 1) ? true : false; }
...
=============
*/
void CL_SetTraceHull(int hull) { clgame.pmove->usehull = bound(0, hull, 3); }
...
=============
*/
movevars_t* pfnGetMoveVars(void) { return &clgame.movevars; }
...
=============
*/
void pfnStopAllSounds(int ent, int entchannel) { S_StopSound(ent, entchannel, NULL); }
...
=============
*/
const char* Key_LookupBinding(const char* pBinding) { return Key_KeynumToString(Key_GetKey(pBinding)); }
...
=============
*/
model_t* pfnLoadMapSprite(const char* filename) { return CL_LoadSpriteModel(filename, SPR_MAPSPRITE, 0); }
...
=============
*/
int pfnGetTrackerIDForPlayer(int playerSlot) { return 0; }
...
=============
*/
int pfnGetPlayerForTrackerID(int trackerID) { return 0; }
...
=============
*/
static float GAME_EXPORT pfnGetClientOldTime(void) { return cl.oldtime; }
...
=============
*/
static float GAME_EXPORT pfnGetGravity(void) { return clgame.movevars.gravity; }
...
=============
*/
static const char* GAME_EXPORT LocalPlayerInfo_ValueForKey(const char* key) { return Info_ValueForKey(cls.userinfo, key); }
...
=============
*/
int TriBoxInPVS(float* mins, float* maxs) { return Mod_BoxVisible(mins, maxs, ref.dllFuncs.Mod_GetCurrentVis()); }
...
=================
*/
static int Demo_IsRecording(void) { return cls.demorecording; }
...
=================
*/
static int Demo_IsPlayingback(void) { return cls.demoplayback; }
...
=================
*/
static int Demo_IsTimeDemo(void) { return cls.timedemo; }
...
=================
*/
static void Demo_WriteBuffer(int size, byte* buffer) { CL_WriteDemoUserMessage(buffer, size); }
...
=================
*/
char* NetAPI_AdrToString(netadr_t* a) { return NET_AdrToString(*a); }
...
=================
*/
int NetAPI_CompareAdr(netadr_t* a, netadr_t* b) { return NET_CompareAdr(*a, *b); }
...
=================
*/
int NetAPI_StringToAdr(char* s, netadr_t* a) { return NET_StringToAdr(s, a); }
...
=================
*/
const char* NetAPI_ValueForKey(const char* s, const char* key) { return Info_ValueForKey(s, key); }
...
=================
*/
void NetAPI_RemoveKey(char* s, const char* key) { Info_RemoveKey(s, key); }
...
=================
*/
int Voice_StartVoiceTweakMode(void) { return 0; }
...
=================
*/
float Voice_GetControlFloat(VoiceTweakControl iControl) { return 1.0f; }
...
public:
	...
	virtual particle_t* AllocParticle(void (*callback)(struct particle_s* particle, float frametime)) { return R_AllocParticle(callback); }

	virtual void BlobExplosion(const float* org) { R_BlobExplosion(org); }

	virtual void Blood(const float* org, const float* dir, int pcolor, int speed) { R_Blood(org, dir, pcolor, speed); }
	...
	virtual void BloodStream(const float* org, const float* dir, int pcolor, int speed) { R_BloodStream(org, dir, pcolor, speed); }

	virtual void BreakModel(const float* pos, const float* size, const float* dir, float random, float life, int count, int modelIndex,
				char flags)
	{
	...
	virtual void BulletImpactParticles(const float* pos) { R_BulletImpactParticles(pos); }

	virtual void EntityParticles(struct cl_entity_s* ent) { R_EntityParticles(ent); }

	virtual void Explosion(float* pos, int model, float scale, float framerate, int flags) { R_Explosion(pos, model, scale, framerate, flags); }

	virtual void FizzEffect(struct cl_entity_s* pent, int modelIndex, int density) { R_FizzEffect(pent, modelIndex, density); }
	...
	virtual void FlickerParticles(const float* org) { R_FlickerParticles(org); }

	virtual void FunnelSprite(const float* org, int modelIndex, int reverse) { R_FunnelSprite(org, modelIndex, reverse); }

	virtual void Implosion(const float* end, float radius, int count, float life) { R_Implosion(end, radius, count, life); }

	virtual void LargeFunnel(const float* org, int reverse) { R_LargeFunnel(org, reverse); }

	virtual void LavaSplash(const float* org) { R_LavaSplash(org); }
	...
	virtual void MuzzleFlash(const float* pos1, int type) { R_MuzzleFlash(pos1, type); }
	...
	virtual void ParticleBurst(const float* pos, int size, int color, float life) { R_ParticleBurst(pos, size, color, life); }

	virtual void ParticleExplosion(const float* org) { R_ParticleExplosion(org); }

	virtual void ParticleExplosion2(const float* org, int colorStart, int colorLength) { R_ParticleExplosion2(org, colorStart, colorLength); }
	...
	virtual void PlayerSprites(int client, int modelIndex, int count, int size) { R_PlayerSprites(client, modelIndex, count, size); }

	virtual void Projectile(const float* origin, const float* velocity, int modelIndex, int life, int owner,
				void (*hitcallback)(struct tempent_s* ent, struct pmtrace_s* ptr))
	{
		R_Projectile(origin, velocity, modelIndex, life, owner, hitcallback);
	}

	virtual void RicochetSound(const float* pos) { R_RicochetSound(pos); }
	...
	virtual void RocketFlare(const float* pos) { R_RocketFlare(pos); }

	virtual void RocketTrail(float* start, float* end, int type) { R_RocketTrail(start, end, type); }

	virtual void RunParticleEffect(const float* org, const float* dir, int color, int count) { R_RunParticleEffect(org, dir, color, count); }

	virtual void ShowLine(const float* start, const float* end) { R_ShowLine(start, end); }
	...
	virtual void SparkShower(const float* pos) { R_SparkShower(pos); }
	...
	virtual void Sprite_Explode(TEMPENTITY* pTemp, float scale, int flags) { R_Sprite_Explode(pTemp, scale, flags); }

	virtual void Sprite_Smoke(TEMPENTITY* pTemp, float scale) { R_Sprite_Smoke(pTemp, scale); }
	...
	virtual void Sprite_Trail(int type, float* start, float* end, int modelIndex, int count, float life, float size, float amplitude,
				  int renderamt, float speed)
	{
		R_Sprite_Trail(type, start, end, modelIndex, count, life, size, amplitude, renderamt, speed);
	}

	virtual void Sprite_WallPuff(TEMPENTITY* pTemp, float scale) { R_Sprite_WallPuff(pTemp, scale); }
	...
	virtual void TracerEffect(const float* start, const float* end) { R_TracerEffect(start, end); }

	virtual void UserTracerParticle(float* org, float* vel, float life, int colorIndex, float length, unsigned char deathcontext,
					void (*deathfunc)(struct particle_s* particle))
	{
		R_UserTracerParticle(org, vel, life, colorIndex, length, deathcontext, deathfunc);
	}

	virtual particle_t* TracerParticles(float* org, float* vel, float life) { return R_TracerParticles(org, vel, life); }

	virtual void TeleportSplash(const float* org) { R_TeleportSplash(org); }
	...
	virtual TEMPENTITY* DefaultSprite(const float* pos, int spriteIndex, float framerate) { return R_DefaultSprite(pos, spriteIndex, framerate); }

	virtual TEMPENTITY* TempSprite(float* pos, const float* dir, float scale, int modelIndex, int rendermode, int renderfx, float a, float life,
				       int flags)
	{
		return R_TempSprite(pos, dir, scale, modelIndex, rendermode, renderfx, a, life, flags);
	}

	virtual int Draw_DecalIndex(int id) { return CL_DecalIndex(id); }

	virtual int Draw_DecalIndexFromName(const char* name) { return CL_DecalIndexFromName(name); }
	...
	virtual void KillAttachedTents(int client) { R_KillAttachedTents(client); }

	virtual BEAM* BeamCirclePoints(int type, float* start, float* end, int modelIndex, float life, float width, float amplitude, float brightness,
	...
	virtual BEAM* BeamEntPoint(int startEnt, float* end, int modelIndex, float life, float width, float amplitude, float brightness, float speed,
	...
	virtual BEAM* BeamEnts(int startEnt, int endEnt, int modelIndex, float life, float width, float amplitude, float brightness, float speed,
			       int startFrame, float framerate, float r, float g, float b)
	{
	...
	virtual void BeamKill(int deadEntity) { R_BeamKill(deadEntity); }
	...
	virtual BEAM* BeamPoints(float* start, float* end, int modelIndex, float life, float width, float amplitude, float brightness, float speed,
				 int startFrame, float framerate, float r, float g, float b)
	{
		return R_BeamPoints(start, end, modelIndex, life, width, amplitude, brightness, speed, startFrame, framerate, r, g, b);
	}

	virtual BEAM* BeamRing(int startEnt, int endEnt, int modelIndex, float life, float width, float amplitude, float brightness, float speed,
			       int startFrame, float framerate, float r, float g, float b)
	{
		return R_BeamRing(startEnt, endEnt, modelIndex, life, width, amplitude, brightness, speed, startFrame, framerate, r, g, b);
	}

	virtual dlight_t* CL_AllocDlight(int key) { return ::CL_AllocDlight(key); }

	virtual dlight_t* CL_AllocElight(int key) { return ::CL_AllocElight(key); }

	virtual TEMPENTITY* CL_TempEntAlloc(const float* org, struct model_s* model) { return ::CL_TempEntAlloc(org, model); }

	virtual TEMPENTITY* CL_TempEntAllocNoModel(const float* org) { return ::CL_TempEntAllocNoModel(org); }

	virtual TEMPENTITY* CL_TempEntAllocHigh(const float* org, struct model_s* model) { return ::CL_TempEntAllocHigh(org, model); }

	virtual TEMPENTITY* CL_TentEntAllocCustom(const float* origin, struct model_s* model, int high,
	...
	virtual void GetPackedColor(short* packed, short color) { R_GetPackedColor(packed, color); }

	virtual short LookupColor(unsigned char r, unsigned char g, unsigned char b) { return R_LookupColor(r, g, b); }

	virtual void DecalRemoveAll(int textureIndex) { CL_DecalRemoveAll(textureIndex); }
...
public:
	...
	virtual void StopSound(int ent, int channel, const char* sample) { S_StopSound(ent, channel, sample); }

	virtual int FindModelIndex(const char* pmodel) { return CL_FindModelIndex(pmodel); }

	virtual int IsLocal(int playernum) { return pfnIsLocal(playernum); }

	virtual int LocalPlayerDucking(void) { return pfnLocalPlayerDucking(); }

	virtual void LocalPlayerViewheight(float* off) { pfnLocalPlayerViewheight(off); }

	virtual void LocalPlayerBounds(int hull, float* mins, float* maxs) { pfnLocalPlayerBounds(hull, mins, maxs); }

	virtual int IndexFromTrace(struct pmtrace_s* pTrace) { return pfnIndexFromTrace(pTrace); }

	virtual struct physent_s* GetPhysent(int idx) { return pfnGetPhysent(idx); }

	virtual void SetUpPlayerPrediction(int dopred, int bIncludeLocalClient) { CL_SetUpPlayerPrediction(dopred, bIncludeLocalClient); }

	virtual void PushPMStates(void) { CL_PushPMStates(); }

	virtual void PopPMStates(void) { CL_PopPMStates(); }

	virtual void SetSolidPlayers(int playernum) { CL_SetSolidPlayers(playernum); }

	virtual void SetTraceHull(int hull) { CL_SetTraceHull(hull); }
	...
	virtual void WeaponAnimation(int sequence, int body) { CL_WeaponAnim(sequence, body); }

	virtual unsigned short PrecacheEvent(int type, const char* psz) { return pfnPrecacheEvent(type, psz); }

	virtual void PlaybackEvent(int flags, const struct edict_s* pInvoker, unsigned short eventindex, float delay, float* origin, float* angles,
	...
	virtual const char* TraceTexture(int ground, float* vstart, float* vend) { return pfnTraceTexture(ground, vstart, vend); }

	virtual void StopAllSounds(int entnum, int entchannel) { pfnStopAllSounds(entnum, entchannel); }

	virtual void KillEvents(int entnum, const char* eventname) { pfnKillEvents(entnum, eventname); }
	...
	virtual const char* SoundForIndex(int index) { return CL_SoundFromIndex(index); }

	virtual struct msurface_s* TraceSurface(int ground, float* vstart, float* vend) { return pfnTraceSurface(ground, vstart, vend); }

	virtual struct movevars_s* GetMovevars(void) { return pfnGetMoveVars(); }

	virtual struct pmtrace_s* VisTraceLine(float* start, float* end, int flags) { return CL_VisTraceLine(start, end, flags); }

	virtual struct physent_s* GetVisent(int idx) { return pfnGetVisent(idx); }

	virtual int TestLine(const vec3_t start, const vec3_t end, int flags) { return CL_TestLine(start, end, flags); }

	virtual void PushTraceBounds(int hullnum, const float* mins, const float* maxs) { CL_PushTraceBounds(hullnum, mins, maxs); }

	virtual void PopTraceBounds(void) { CL_PopTraceBounds(); }
};
...
public:
	...
	virtual int IsRecording(void) { return Demo_IsRecording(); }

	virtual int IsPlayingback(void) { return Demo_IsPlayingback(); }

	virtual int IsTimeDemo(void) { return Demo_IsTimeDemo(); }

	virtual void WriteBuffer(int size, unsigned char* buffer) { Demo_WriteBuffer(size, buffer); }
};
...
public:
	...
	virtual void InitNetworking(void) { NetAPI_InitNetworking(); }
	virtual void Status(struct net_status_s* status) { NetAPI_Status(status); }
	virtual void SendRequest(int context, int request, int flags, double timeout, struct netadr_s* remote_address,
				 net_api_response_func_t response)
	{
		NetAPI_SendRequest(context, request, flags, timeout, remote_address, response);
	}
	virtual void	    CancelRequest(int context) { NetAPI_CancelRequest(context); }
	virtual void	    CancelAllRequests(void) { NetAPI_CancelAllRequests(); }
	virtual char*	    AdrToString(struct netadr_s* a) { return NetAPI_AdrToString(a); }
	virtual int	    CompareAdr(struct netadr_s* a, struct netadr_s* b) { return NetAPI_CompareAdr(a, b); }
	virtual int	    StringToAdr(char* s, struct netadr_s* a) { return NetAPI_StringToAdr(s, a); }
	virtual const char* ValueForKey(const char* s, const char* key) { return NetAPI_ValueForKey(s, key); }
	virtual void	    RemoveKey(char* s, const char* key) { return NetAPI_RemoveKey(s, key); }
	virtual void SetValueForKey(char* s, const char* key, const char* value, int maxsize) { NetAPI_SetValueForKey(s, key, value, maxsize); }
};
EXPOSE_INTERFACE(CEngineNetAPI);

static net_api_t gNetApi = {
	NetAPI_InitNetworking, NetAPI_Status,	   NetAPI_SendRequest, NetAPI_CancelRequest, NetAPI_CancelAllRequests, NetAPI_AdrToString,
	NetAPI_CompareAdr,     NetAPI_StringToAdr, NetAPI_ValueForKey, NetAPI_RemoveKey,     NetAPI_SetValueForKey,
};
...
static cl_enginefunc_t gEngfuncs = {
	pfnSPR_Load, pfnSPR_Frames, pfnSPR_Height, pfnSPR_Width, pfnSPR_Set, pfnSPR_Draw, pfnSPR_DrawHoles, pfnSPR_DrawAdditive, SPR_EnableScissor,
	SPR_DisableScissor, pfnSPR_GetList, CL_FillRGBA, CL_GetScreenInfo, pfnSetCrosshair, pfnCvar_RegisterClientVariable, Cvar_VariableValue,
	Cvar_VariableString, Cmd_AddClientCommand, pfnHookUserMsg, pfnServerCmd, pfnClientCmd, pfnGetPlayerInfo, pfnPlaySoundByName,
	pfnPlaySoundByIndex, AngleVectors, CL_TextMessageGet, pfnDrawCharacter, pfnDrawConsoleString, pfnDrawSetTextColor, pfnDrawConsoleStringLen,
	pfnConsolePrint, pfnCenterPrint, pfnGetWindowCenterX, pfnGetWindowCenterY, pfnGetViewAngles, pfnSetViewAngles, CL_GetMaxClients,
	Cvar_SetValue, Cmd_Argc, Cmd_Argv, Con_Printf, Con_DPrintf, Con_NPrintf, Con_NXPrintf, pfnPhysInfo_ValueForKey, pfnServerInfo_ValueForKey,
	pfnGetClientMaxspeed, COM_CheckParm, Key_Event, Platform_GetMousePos, pfnIsNoClipping, CL_GetLocalPlayer, CL_GetViewModel,
	CL_GetEntityByIndex, pfnGetClientTime, pfnCalcShake, pfnApplyShake, pfnPointContents, CL_WaterEntity, pfnTraceLine, CL_LoadModel,
	CL_AddEntity, CL_GetSpritePointer, pfnPlaySoundByNameAtLocation, pfnPrecacheEvent, CL_PlaybackEvent, CL_WeaponAnim, COM_RandomFloat,
	COM_RandomLong, pfnHookEvent, Con_Visible, pfnGetGameDirectory, pfnCVarGetPointer, Key_LookupBinding, pfnGetLevelName, pfnGetScreenFade,
	pfnSetScreenFade, VGui_GetPanel, VGui_ViewportPaintBackground, COM_LoadFile, pfnParseFile, COM_FreeFile, &gTriApi, &gEfxApi, &gEventApi,
	&gDemoApi, &gNetApi, &gVoiceApi, pfnIsSpectateOnly, pfnLoadMapSprite, COM_AddAppDirectoryToSearchPath, COM_ExpandFilename,
	PlayerInfo_ValueForKey, PlayerInfo_SetValueForKey, pfnGetPlayerUniqueID, pfnGetTrackerIDForPlayer, pfnGetPlayerForTrackerID,
	pfnServerCmdUnreliable, pfnGetMousePos, Platform_SetMousePos, pfnSetMouseEnable, Cvar_GetList,
	reinterpret_cast<void* (*)(void)>((void*)Cmd_GetFirstFunctionHandle), reinterpret_cast<void* (*)(void*)>((void*)Cmd_GetNextFunctionHandle),
	reinterpret_cast<const char* (*)(void*)>((void*)Cmd_GetName), pfnGetClientOldTime, pfnGetGravity, Mod_Handle, pfnEnableTexSort,
	pfnSetLightmapColor, pfnSetLightmapScale, pfnSequenceGet, pfnSPR_DrawGeneric, pfnSequencePickSentence, pfnDrawString, pfnDrawStringReverse,
	LocalPlayerInfo_ValueForKey, pfnVGUI2DrawCharacter, pfnVGUI2DrawCharacterAdditive,
	reinterpret_cast<unsigned int (*)(char*)>((void*)Sound_GetApproxWavePlayLen), GetCareerGameInterface,
	reinterpret_cast<void (*)(char*, char*)>((void*)Cvar_Set), pfnIsCareerMatch, pfnPlaySoundVoiceByName, pfnMP3_InitStream, Sys_DoubleTime,
	pfnProcessTutorMessageDecayBuffer, pfnConstructTutorMessageDecayBuffer, pfnResetTutorMessageDecayData, pfnPlaySoundByNameAtPitch,
	CL_FillRGBABlend, pfnGetAppID, Cmd_AliasGetList, pfnVguiWrap2_GetMouseDelta,
...
qboolean CL_LoadProgs(const char* name)
{
	...
	if ((GetClientAPI = reinterpret_cast<CL_EXPORT_FUNCS>((void*)COM_GetProcAddress(clgame.hInstance, "GetClientAPI"))) != NULL)
	{
