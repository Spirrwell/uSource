...
static world_static_t* pfnGetWorld(void) { return &world; }

static void pfnStudioEvent(const mstudioevent_t* event, const cl_entity_t* e) { clgame.dllFuncs.pfnStudioEvent(event, e); }

static efrag_t* pfnGetEfragsFreeList(void) { return clgame.free_efrags; }

static void pfnSetEfragsFreeList(efrag_t* list) { clgame.free_efrags = list; }
...
static model_t* pfnMod_GetCurrentLoadingModel(void) { return loadmodel; }

static void pfnMod_SetCurrentLoadingModel(model_t* m) { loadmodel = m; }

static void pfnGetPredictedOrigin(vec3_t v) { VectorCopy(cl.simorg, v); }
...
static void pfnSetLocalLightLevel(int level) { cl.local.light_level = level; }
...
{
	return clgame.dllFuncs.pfnGetStudioModelInterface ? clgame.dllFuncs.pfnGetStudioModelInterface(version, ppinterface, pstudio) : 0;
}

static byte* pfnImage_GetPool(void) { return host.imagepool; }

static const bpc_desc_t* pfnImage_GetPFDesc(int idx) { return &PFDesc[idx]; }

static void pfnDrawNormalTriangles(void) { clgame.dllFuncs.pfnDrawNormalTriangles(); }

static void pfnDrawTransparentTriangles(void) { clgame.dllFuncs.pfnDrawTransparentTriangles(); }

static screenfade_t* pfnRefGetScreenFade(void) { return &clgame.fade; }
...
static ref_api_t gEngfuncs = {pfnEngineGetParm,
...
#else
		Q_snprintf(dest, size, "%sref_%s.%s", OS_LIB_PREFIX, opt, OS_LIB_EXT);
#endif
...
qboolean R_Init(void)
{
	...
	if (!Sys_GetParmFromCmdLine("-ref", refopt))
	{
		...
		Q_strncpy(refopt, COM_CheckString(r_refdll->string) ? r_refdll->string : DEFAULT_ACCELERATED_RENDERER, sizeof(refopt));
	}
