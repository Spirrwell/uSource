...
==================
*/
qboolean CL_IsPlayerIndex(int idx) { return (idx >= 1 && idx <= cl.maxclients); }
...
qboolean CL_GetMovieSpatialization(rawchan_t* ch)
{
	...
	// setup radius
	if (ent->model != NULL && ent->model->radius)
		ch->radius = ent->model->radius;
	else
		ch->radius = RadiusFromBounds(ent->curstate.mins, ent->curstate.maxs);
