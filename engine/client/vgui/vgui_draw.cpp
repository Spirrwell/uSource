...
void GAME_EXPORT* VGUI_EngineMalloc(size_t size) { return Z_Malloc(size); }

qboolean GAME_EXPORT VGUI_IsInGame(void) { return cls.state == ca_active && cls.key_dest == key_game; }
...
byte GAME_EXPORT VGUI_GetColor(int i, int j) { return g_color_table[i][j]; }
...
qboolean VGui_IsActive(void) { return vgui.initialized; }
