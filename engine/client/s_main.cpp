...
=================
*/
qboolean S_IsClient(int entnum) { return (entnum == s_listener.entnum); }
...
===================
*/
static uint S_RawSamplesStereo(portable_samplepair_t* rawsamples, uint rawend, uint max_samples, uint samples, uint rate, word width, word channels,
			       const byte* data)
{
...
void S_RawSamples(uint samples, uint rate, word width, word channels, const byte* data, int entnum)
{
	...
	if (snd_vol < 0)
		snd_vol = 0; // fixup negative values
...
void S_StreamAviSamples(void* Avi, int entnum, float fvol, float attn, float synctime)
{
	...
	while (ch->s_rawend < soundtime + ch->max_samples)
	{
		...
		{
			// add to raw buffer
			ch->s_rawend = S_RawSamplesStereo(ch->rawsamples, ch->s_rawend, ch->max_samples, fileSamples, info->rate, info->width,
							  info->channels, raw);
		}
		else
...
void SND_UpdateSound(void)
{
	...
	if (CVAR_TO_BOOL(s_show))
	{
		...
		for (i = 0, total = 1, ch = channels; i < MAX_CHANNELS; i++, ch++)
		{
			...
			{
				info.index = total;
				Con_NXPrintf(&info, "chan %i, pos (%.f %.f %.f) ent %i, lv%3i rv%3i %s\n", i, ch->origin[0], ch->origin[1],
					     ch->origin[2], ch->entnum, ch->leftvol, ch->rightvol, ch->sfx->name);
				total++;
			}
		}
...
=================
*/
void S_StopSound_f(void) { S_StopAllSounds(true); }
