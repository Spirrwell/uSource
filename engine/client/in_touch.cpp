...
void Touch_WriteConfig(void)
{
	...
	if (f)
	{
		...
		for (button = touch.list_user.first; button; button = button->next)
		{
			...
			FS_Printf(f, "touch_addbutton \"%s\" \"%s\" \"%s\" %f %f %f %f %d %d %d %d %d\n", B(name), B(texturefile), newCommand, B(x1),
				  B(y1), B(x2), B(y2), B(color[0]), B(color[1]), B(color[2]), B(color[3]), flags);
		}
...
void Touch_ExportConfig_f(void)
{
	...
	if (f)
	{
		...
		if (Q_strstr(name, "touch_presets/"))
		{
			...
			Q_strncpy(profilename, name, 256);
		FS_Printf(f, "//=======================================================================\n");
		FS_Printf(f, "//\tCopyright SDLash3D team & XashXT group %s ©\n", Q_timestamp(TIME_YEAR_ONLY));
		...
		for (button = touch.list_user.first; button; button = button->next)
		{
			...
			FS_Printf(f, "touch_addbutton \"%s\" \"%s\" \"%s\" %f %f %f %f %d %d %d %d %d %f\n", B(name), B(texturefile), newCommand,
				  B(x1), B(y1), B(x2), B(y2), B(color[0]), B(color[1]), B(color[2]), B(color[3]), flags, aspect);
		}
...
void Touch_ListButtons_f(void)
{
	...
	for (button = touch.list_user.first; button; button = button->next)
	{
		Con_Printf("%s %s %s %f %f %f %f %d %d %d %d %d\n", B(name), B(texturefile), B(command), B(x1), B(y1), B(x2), B(y2), B(color[0]),
			   B(color[1]), B(color[2]), B(color[3]), B(flags));
...
void Touch_SetClientOnly_f(void) { Touch_SetClientOnly(Q_atoi(Cmd_Argv(1))); }
...
void Touch_RemoveButton(const char* name) { Touch_RemoveButtonFromList(&touch.list_user, name); }

void IN_TouchRemoveButton_f(void) { Touch_RemoveButton(Cmd_Argv(1)); }
...
void Touch_Hide_f(void) { Touch_HideButtons(Cmd_Argv(1), true); }

void Touch_Show_f(void) { Touch_HideButtons(Cmd_Argv(1), false); }
...
touch_button_t* Touch_AddButton(touchbuttonlist_t* list, const char* name, const char* texture, const char* command, float x1, float y1, float x2,
				float y2, byte* color)
{
...
void Touch_AddClientButton(const char* name, const char* texture, const char* command, float x1, float y1, float x2, float y2, byte* color, int round,
			   float aspect, int flags)
{
	touch_button_t* button;
...
void Touch_LoadDefaults_f(void)
{
	...
	{
		touch_button_t* button;
		float		x1 = g_DefaultButtons[i].x1, y1 = g_DefaultButtons[i].y1, x2 = g_DefaultButtons[i].x2, y2 = g_DefaultButtons[i].y2;
		...
		if (g_DefaultButtons[i].aspect && g_DefaultButtons[i].round == round_aspect)
		{
			if (g_DefaultButtons[i].texturefile[0] == '#')
				y2 = y1 + ((float)clgame.scrInfo.iCharHeight / (float)clgame.scrInfo.iHeight) * g_DefaultButtons[i].aspect +
		...
		IN_TouchCheckCoords(&x1, &y1, &x2, &y2);
		button = Touch_AddButton(&touch.list_user, g_DefaultButtons[i].name, g_DefaultButtons[i].texturefile, g_DefaultButtons[i].command, x1,
					 y1, x2, y2, g_DefaultButtons[i].color);
...
void Touch_AddDefaultButton(const char* name, const char* texturefile, const char* command, float x1, float y1, float x2, float y2, byte* color,
			    int round, float aspect, int flags)
{
...
void Touch_ResetDefaultButtons(void) { g_LastDefaultButton = 0; }
void Touch_AddButton_f(void)
{
	...
	if (argc >= 12)
	{
		...
		button = Touch_AddButton(&touch.list_user, Cmd_Argv(1), Cmd_Argv(2), Cmd_Argv(3), Q_atof(Cmd_Argv(4)), Q_atof(Cmd_Argv(5)),
					 Q_atof(Cmd_Argv(6)), Q_atof(Cmd_Argv(7)), color);
		if (argc >= 13)
	...
	if (argc == 8)
	{
		...
		Touch_AddButton(&touch.list_user, Cmd_Argv(1), Cmd_Argv(2), Cmd_Argv(3), Q_atof(Cmd_Argv(4)), Q_atof(Cmd_Argv(5)),
				Q_atof(Cmd_Argv(6)), Q_atof(Cmd_Argv(7)), color);
		return;
	}
	if (argc == 4)
...
void Touch_InitEditor(void)
{
	...
	Touch_AddButton(&touch.list_edit, "close", "touch_default/edit_close.tga", "touch_disableedit", 0, y, x, y + 0.1, (byte*)"\xff\xff\xff\xff")
		->flags |= TOUCH_FL_NOEDIT;
	Touch_AddButton(&touch.list_edit, "close", "#Close and save", "", x, y, x + 0.2, y + 0.1, (byte*)"\xff\xff\xff\xff")->flags |=
		TOUCH_FL_NOEDIT;
	y += 0.2;
	Touch_AddButton(&touch.list_edit, "cancel", "touch_default/edit_reset.tga", "touch_reloadconfig", 0, y, x, y + 0.1, (byte*)"\xff\xff\xff\xff")
		->flags |= TOUCH_FL_NOEDIT;
	Touch_AddButton(&touch.list_edit, "close", "#Cancel and reset", "", x, y, x + 0.2, y + 0.1, (byte*)"\xff\xff\xff\xff")->flags |=
		TOUCH_FL_NOEDIT;
	y += 0.2;
	touch.hidebutton = Touch_AddButton(&touch.list_edit, "showhide", "touch_default/edit_hide.tga", "touch_toggleselection", 0, y, x, y + 0.1,
...
void Touch_Init(void)
{
	...
	Touch_AddDefaultButton("flashlight", "touch_default/flash_light_filled.tga", "impulse 100", 0.920000, 0.000000, 1.000000, 0.151486, color, 2,
			       1, 0);
	Touch_AddDefaultButton("scores", "touch_default/map.tga", "+showscores", 0.760000, 0.000000, 0.840000, 0.151486, color, 2, 1, 8);
	Touch_AddDefaultButton("show_numbers", "touch_default/show_weapons.tga", "exec touch_default/numbers.cfg", 0.440000, 0.833171, 0.520000,
...
void Touch_DrawTexture(float x1, float y1, float x2, float y2, int texture, byte r, byte g, byte b, byte a)
{
	...
	ref.dllFuncs.R_DrawStretchPic(TO_SCRN_X(x1), TO_SCRN_Y(y1), TO_SCRN_X(x2 - x1), TO_SCRN_Y(y2 - y1), 0, 0, 1, 1, texture);
}
...
float Touch_DrawText(float x1, float y1, float x2, float y2, const char* s, byte* color, float size)
{
	...
	ref.dllFuncs.Color4ub(color[0] * ((float)color[3] / 255.0f), color[1] * ((float)color[3] / 255.0f), color[2] * ((float)color[3] / 255.0f),
			      255);
	...
	for (button = list->first; button; button = button->next)
	{
		if (Touch_IsVisible(button))
		{
			...
			if (button->texturefile[0] == '#')
				Touch_DrawText(touch.swidth / SCR_W + B(x1), touch.swidth / SCR_H + B(y1), B(x2), B(y2), button->texturefile + 1,
					       color, B(aspect) ? B(aspect) : 1);
			...
			if (B(flags) & TOUCH_FL_STROKE)
			{
				...
				ref.dllFuncs.R_DrawStretchPic(TO_SCRN_X(B(x1)), TO_SCRN_Y(B(y1)), touch.swidth,
							      TO_SCRN_Y(B(y2) - B(y1)) - touch.swidth, 0, 0, 1, 1, touch.whitetexture);
				ref.dllFuncs.R_DrawStretchPic(TO_SCRN_X(B(x1)) + touch.swidth, TO_SCRN_Y(B(y1)),
							      TO_SCRN_X(B(x2) - B(x1)) - touch.swidth, touch.swidth, 0, 0, 1, 1, touch.whitetexture);
				ref.dllFuncs.R_DrawStretchPic(TO_SCRN_X(B(x2)) - touch.swidth, TO_SCRN_Y(B(y1)) + touch.swidth, touch.swidth,
							      TO_SCRN_Y(B(y2) - B(y1)) - touch.swidth, 0, 0, 1, 1, touch.whitetexture);
				ref.dllFuncs.R_DrawStretchPic(TO_SCRN_X(B(x1)), TO_SCRN_Y(B(y2)) - touch.swidth,
							      TO_SCRN_X(B(x2) - B(x1)) - touch.swidth, touch.swidth, 0, 0, 1, 1, touch.whitetexture);
...
void Touch_Draw(void)
{
	...
	if (touch.state >= state_edit)
	{
		...
		if (touch.selection)
		{
			...
			Con_DrawString(Con_DrawString(0, TO_SCRN_Y(GRID_Y * 12), "Name: ", color), TO_SCRN_Y(GRID_Y * 12), B(name), color);
			Con_DrawString(Con_DrawString(0, TO_SCRN_Y(GRID_Y * 13), "Texture: ", color), TO_SCRN_Y(GRID_Y * 13), B(texturefile), color);
			Con_DrawString(Con_DrawString(0, TO_SCRN_Y(GRID_Y * 14), "Command: ", color), TO_SCRN_Y(GRID_Y * 14), B(command), color);
		}
	}
	...
	if ((touch.move_finger != -1) && touch.move && touch_move_indicator->value)
	{
		...
		ref.dllFuncs.R_DrawStretchPic(TO_SCRN_X(touch.move_start_x - GRID_X * touch_move_indicator->value),
					      ...
					      TO_SCRN_Y(GRID_Y * 2 * touch_move_indicator->value), 0, 0, 1, 1, touch.joytexture);
		ref.dllFuncs.Color4ub(255, 255, 255, 255);
...
static void Touch_Motion(touchEventType type, int fingerID, float x, float y, float dx, float dy)
{
	...
	if (fingerID == touch.move_finger)
	{
		...
		else if (touch.move->type == touch_dpad)
		{
			...
			touch.forward =
				round(((touch.move->y2 + touch.move->y1) - y * 2) / (touch.move->y2 - touch.move->y1) * touch_dpad_radius->value);
			touch.side =
				round((x * 2 - (touch.move->x2 + touch.move->x1)) / (touch.move->x2 - touch.move->x1) * touch_dpad_radius->value);
		}
			...
			if ((x > button->x1 && x < button->x2) && (y < button->y2 && y > button->y1))
			{
				...
				if (button->type == touch_move || button->type == touch_joy || button->type == touch_dpad)
				{
					...
					else if (touch.move->type == touch_dpad)
					{
						...
						// start move instanly
						touch.forward =
							round(((touch.move->y2 + touch.move->y1) - y * 2) / (touch.move->y2 - touch.move->y1));
