...
================
*/
void Con_PageUp(int lines) { con.backscroll += abs(lines); }
...
================
*/
void Con_PageDown(int lines) { con.backscroll -= abs(lines); }
...
================
*/
void Con_Top(void) { con.backscroll = CON_MAXLINES; }
...
================
*/
void Con_Bottom(void) { con.backscroll = 0; }
...
================
*/
int Con_Visible(void) { return (con.vislines > 0); }
...
int table_cp1251[64] = {0x0402, 0x0403, 0x201A, 0x0453, 0x201E, 0x2026, 0x2020, 0x2021, 0x20AC, 0x2030, 0x0409, 0x2039, 0x040A,
...
====================
*/
void Con_RestoreFont(void) { con.curFont = con.lastUsedFont; }
...
{
	if (width && con.curFont)
		*width = con.curFont->charWidths[number];
	if (height && con.curFont)
	...
	*(uint*)color = *(uint*)setColor;
	...
	while (*s)
	{
		...
		if (hideChar == numDraws)
			...
			drawLen += Con_DrawCharacter(x + drawLen, y, *s, color);
...
====================
*/
int Con_DrawString(int x, int y, const char* string, rgba_t setColor) { return Con_DrawGenericString(x, y, string, setColor, false, -1); }
...
void Con_DrawDebug(void)
{
	...
	if (scr_download->value != -1.0f)
	{
		...
		Q_snprintf(dlstring, sizeof(dlstring), "Downloading [%d remaining]: ^2%s^7 %5.1f%% time %.f secs", host.downloadcount,
			   host.downloadfile, scr_download->value, Sys_DoubleTime() - timeStart);
...
void Con_DrawSolidConsole(int lines)
{
	...
	if (host.allow_console)
	{
		...
		Q_snprintf(curbuild, MAX_STRING, "%s %i/%s (%s-%s build %i)", XASH_ENGINE_NAME, PROTOCOL_VERSION, XASH_VERSION, Q_buildos(),
...
void Con_DrawVersion(void)
{
	...
	if (host.force_draw_version || draw_version)
		Q_snprintf(curbuild, MAX_STRING, "%s v%i/%s (%s-%s build %i)", XASH_ENGINE_NAME, PROTOCOL_VERSION, XASH_VERSION, Q_buildos(),
			   Q_buildarch(), Q_buildnum());
	else
		Q_snprintf(curbuild, MAX_STRING, "v%i/%s (%s-%s build %i)", PROTOCOL_VERSION, XASH_VERSION, Q_buildos(), Q_buildarch(), Q_buildnum());
...
void Con_VidInit(void)
{
	...
	if (!con.background) // last chance - quake conback image
	{
		...
		if (draw_to_console && con.curFont && (buf = ref.dllFuncs.R_GetTextureOriginalBuffer(con.curFont->hFontTexture)) != NULL)
		{
