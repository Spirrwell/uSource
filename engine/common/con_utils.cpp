...
=================
*/
static int Con_SortCmds(const void* arg1, const void* arg2) { return Q_stricmp(*(const char**)arg1, *(const char**)arg2); }
...
qboolean Cmd_GetCDList(const char* s, char* completedname, int length)
{
	...
	const char* cd_command[] = {
		"info", "loop", "off", "on", "pause", "play", "resume", "stop",
	};
...
int Cmd_CheckMapsList(int fRefresh) { return Cmd_CheckMapsList_R(fRefresh, true); }
...
void Cmd_WriteOpenGLVariables(file_t* f) { Cvar_LookupVars(FCVAR_GLCONFIG, NULL, f, (setpair_t)Cmd_WriteOpenGLCvar); }
