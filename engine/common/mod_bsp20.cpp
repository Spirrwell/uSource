...
const char* g_bsp20_extensions[] = {
	"bsp", "bsp20", /* we can explicitly specify the map version using this */
};
;
...
CBsp20Loader::CBsp20Loader() : IMapLoader(g_bsp20_extensions, ARRAYSIZE(g_bsp20_extensions)) {}

bool CBsp20Loader::CheckBuffer(const void* buf, size_t len)
{
	if (len < sizeof(bsp20::dheader_t) || !buf)
		return false;
