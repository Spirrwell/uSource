...
static int ioctl_stub(int d, unsigned long r, ...) { return 0; }
...
====================
*/
qboolean NET_IsLocalAddress(netadr_t adr) { return (adr.type == NA_LOOPBACK) ? true : false; }
...
qboolean NET_QueuePacket(netsrc_t sock, netadr_t* from, byte* data, size_t* length)
{
	...
	*length = 0;
	...
	if (NET_IsSocketValid(net_socket))
	{
		...
		ret	 = recvfrom(net_socket, buf, sizeof(buf), 0, (struct sockaddr*)&addr, reinterpret_cast<socklen_t*>(&addr_len));
		...
		while (len > 0)
		{
			...
			if (net_showpackets && net_showpackets->value == 3.0f)
			{
				...
				Con_Printf("Sending split %i of %i with %i bytes and seq %i to %s\n", packet_number + 1, packet_count, size,
					   net.sequence_number, NET_AdrToString(adr));
			}
...
==================
*/
void NET_SendPacket(netsrc_t sock, size_t length, const void* data, netadr_t to) { NET_SendPacketEx(sock, length, data, to, 0); }
...
static int NET_Isocket(const char* net_interface, int port, qboolean multicast)
{
	...
	if (!net_interface[0] || !Q_stricmp(net_interface, "localhost"))
		...
		NET_StringToSockaddr(net_interface, (struct sockaddr*)&addr, false);
	...
	if (net.allow_ip)
	{
		...
		if (NET_StringToAdr(buff, &net_local))
		{
			...
			if (NET_IsSocketError(
				    getsockname(net.ip_sockets[NS_SERVER], (struct sockaddr*)&address, reinterpret_cast<socklen_t*>(&namelen))))
			{
...
====================
*/
qboolean NET_IsConfigured(void) { return net.configured; }
...
#ifndef XASH_DEDICATED
				UI_ConnectionProgress_Download(curfile->path, curfile->server->host, curfile->server->path, curfile->id,
							       http.fileCount, va("(file size is %d)", curfile->size));
...
#ifndef XASH_DEDICATED
				UI_ConnectionProgress_Download(curfile->path, curfile->server->host, curfile->server->path, curfile->id,
							       http.fileCount, va("(file size is %d, speed is %.2f KB/s)", curfile->size, speed));
#endif // XASH_DEDICATED
			}
		}
	}
	...
	for (curfile = http.first_file; curfile; curfile = curfile->next)
	{
...
#ifndef XASH_DEDICATED
			UI_ConnectionProgress_Download(curfile->path, curfile->server->host, curfile->server->path, curfile->id, http.fileCount,
						       "(starting)");
#endif // XASH_DEDICATED
		...
		if (curfile->state < HTTP_REQUEST) // Request not formatted
		{
			curfile->query_length = Q_snprintf(curfile->buf, sizeof(curfile->buf),
							   ...
							   "Host: %s\r\n"
							   "User-Agent: %s\r\n\r\n",
							   curfile->server->path, curfile->path, curfile->server->host, http_useragent->string);
...
static void HTTP_List_f(void)
{
	...
	{
		if (file->server)
			Con_Printf("\t%d %d http://%s:%d/%s%s %d\n", file->id, file->state, file->server->host, file->server->port,
				   file->server->path, file->path, file->downloaded);
		else
