...
#define synchsafe_to_long(buf, res)                                                                                                                  \
	((((buf)[0] | (buf)[1] | (buf)[2] | (buf)[3]) & 0x80)                                                                                        \
		 ? 0                                                                                                                                 \
		 : (res = (((ulong)(buf)[0]) << 21) | (((ulong)(buf)[1]) << 14) | (((ulong)(buf)[2]) << 7) | ((ulong)(buf)[3]), 1))
...
int frame_bitrate(mpg123_handle_t* fr) { return tabsel_123[fr->lsf][fr->lay - 1][fr->bitrate_index]; }

long frame_freq(mpg123_handle_t* fr) { return freqs[fr->sampling_frequency]; }
...
static int header_mono(ulong newhead) { return HDR_CHANNEL_VAL(newhead) == MPG_MD_MONO ? TRUE : FALSE; }
...
static int check_lame_tag(mpg123_handle_t* fr)
{
	...
	if ((fr->bsbuf[lame_offset] == 'I') && (fr->bsbuf[lame_offset + 1] == 'n') && (fr->bsbuf[lame_offset + 2] == 'f') &&
	    (fr->bsbuf[lame_offset + 3] == 'o'))
	{
		// we still have to see what there is
	}
	else if ((fr->bsbuf[lame_offset] == 'X') && (fr->bsbuf[lame_offset + 1] == 'i') && (fr->bsbuf[lame_offset + 2] == 'n') &&
...
init_resync:
	...
	if (!(fr->state_flags & FRAME_FRANKENSTEIN) &&
	    ((fr->track_frames > 0 && fr->num >= fr->track_frames) || (fr->gapless_frames > 0 && fr->num >= fr->gapless_frames)))
