...
static const loadwavfmt_t load_game[] = {{DEFAULT_SOUNDPATH "%s%s.%s", "wav", Sound_LoadWAV},
...
static const streamfmt_t stream_game[] = {{"%s%s.%s", "mp3", Stream_OpenMPG, Stream_ReadMPG, Stream_SetPosMPG, Stream_GetPosMPG, Stream_FreeMPG},
		...
		else
		{
			for (i = 0; i < outcount; i++)
			{
				...
				if (inwidth == 2)
					...
					sample = (int)((char)(data[srcsample])) << 8;

				if (outwidth == 2)
					((short*)sound.tempbuffer)[i] = sample;
				else
					((signed char*)sound.tempbuffer)[i] = sample >> 8;
			}
		}
