...
============
*/
cvar_t* GAME_EXPORT Cvar_GetList(void) { return (cvar_t*)cvar_vars; }
	...
	for (var = cvar_vars; var; var = var->next)
	{
		...
		else
		{
			...
			if (FBitSet(var->flags, FCVAR_ALLOCATED | FCVAR_EXTENDED))
				...
				callback(var->name, var->string, (void*)"", ptr);
		}
	}
}
...
void Cvar_DirectSet(convar_t* var, const char* value)
{
	...
	if (var->callback)
		var->callback(var->string, pszValue);
...
void CEngineCvar::CvarSetString(const char* name, const char* string) { Cvar_Set(name, string); }

void CEngineCvar::CmdInit() { Cmd_Init(); }

void CEngineCvar::CvarInit() { Cvar_Init(); }

int CEngineCvar::CmdArgc() { return Cmd_Argc(); }

const char** CEngineCvar::CmdArgv() { return Cmd_Argl(); }
