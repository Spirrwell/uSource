...
IModelLoader::IModelLoader(const char* const* extensions, int nextensions)
{
	...
	model_loader_t loader		     = {extensions, nextensions, this};
...
IMapLoader::IMapLoader(const char* const* extensions, int nextensions)
{
	...
	map_loader_t loader		 = {extensions, nextensions, this};
...
model_t* Mod_LoadModel(model_t* mod, qboolean crash, EModelType type)
{
	...
	case EModelType::NORMAL:
		for (int i = 0; i < g_num_model_loaders; i++)
		{
			...
			{
				if (Q_strcmp(COM_FileExtension(mod->name), ldr.extensions[j]) == 0 && ldr.loader->CheckBuffer(buf, length))
				{
					/* Use this loader */
...
model_t* Mod_LoadWorld(const char* name, qboolean preload)
{
	...
	if (preload)
		Mod_LoadModel(pworld, true, EModelType::MAP);
	world.loading = false;
...
===============
*/
void* Mod_CacheCheck(cache_user_t* c) { return Cache_Check(com_studiocache, c); }
