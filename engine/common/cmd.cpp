...
===============
*/
void Cmd_StuffCmds_f(void) { host.stuffcmds_pending = true; }
...
============
*/
const char** Cmd_Argl() { return cmd_argv; }
...
============
*/
cmdalias_t* GAME_EXPORT Cmd_AliasGetList(void) { return cmd_alias; }
...
============
*/
cmd_t* GAME_EXPORT Cmd_GetFirstFunctionHandle(void) { return cmd_functions; }
...
============
*/
cmd_t* GAME_EXPORT Cmd_GetNextFunctionHandle(cmd_t* cmd) { return (cmd) ? cmd->next : NULL; }
...
============
*/
char* GAME_EXPORT Cmd_GetName(cmd_t* cmd) { return cmd->name; }
...
============
*/
static int Cmd_AddCommandEx(const char* funcname, const char* cmd_name, xcommand_t function, const char* cmd_desc, int iFlags)
{
	cmd_t *cmd, *cur, *prev;
...
============
*/
void Cmd_Else_f(void) { cmd_condition ^= BIT(cmd_condlevel); }
	...
	if (cmd_scripting && cmd_scripting->value)
	{
		while (*text)
		{
			...
			else if (*text == '$')
			{
				...
				text++;
				while ((*text >= '0' && *text <= '9') || (*text >= 'A' && *text <= 'Z') || (*text >= 'a' && *text <= 'z') ||
				       (*text == '_'))
					*ptoken++ = *text++;
				*ptoken = 0;
...
static void Cmd_Apropos_f(void)
{
	...
	for (var = (convar_t*)Cvar_GetList(); var; var = var->next)
	{
		...
		Msg("cvar ^3%s^7 is \"%s\" [\"%s\"] %s\n", var->name, var->string, (var->flags & FCVAR_EXTENDED) ? var->def_string : "",
