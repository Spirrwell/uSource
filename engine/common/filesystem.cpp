...
static dpackfile_t* FS_AddFileToPack(const char* name, pack_t* pack, fs_offset_t offset, fs_offset_t size)
{
	...
	while (left <= right)
	{
		...
		if (!diff)
			Con_Reportf(S_WARN "package %s contains the file %s several times\n", pack->filename, name);
...
void FS_Path_f(void)
{
	...
	{
		if (s->pack)
			Con_Printf("%s (%i files)", s->pack->filename, s->pack->numfiles);
		else if (s->wad)
			Con_Printf("%s (%i files)", s->wad->filename, s->wad->numlumps);
		else if (s->zip)
			Con_Printf("%s (%i files)", s->zip->filename, s->zip->numfiles);
		else
			Con_Printf("%s", s->filename);
...
============
*/
void FS_ClearPaths_f(void) { FS_ClearSearchPath(); }
...
================
*/
void FS_Rescan_f(void) { FS_Rescan(); }
...
void FS_ParseGenericGameInfo(gameinfo_t* GameInfo, const char* buf, const qboolean isGameInfo)
{
	...
	if (GameInfo->game_dll_linux[0])
		found_linux = true;
	if (GameInfo->game_dll_osx[0])
		found_osx = true;
...
void FS_AllowDirectPaths(qboolean enable) { fs_ext_path = enable; }
...
====================
*/
int FS_Print(file_t* file, const char* msg) { return FS_Write(file, msg, Q_strlen(msg)); }
...
dll_user_t* FS_FindLibrary(const char* dllname, qboolean directpath)
{
	...
	for (i = 0; i < Q_strlen(dllname); i++)
	{
		if (dllname[i + start] == '\\')
			...
			dllpath[i] = Q_tolower(dllname[i + start]);
	}
	dllpath[i] = '\0';
...
search_t* FS_Search(const char* pattern, int caseinsensitive, int gamedironly)
{
	...
	if (resultlist.numstrings)
	{
		...
		search			= static_cast<search_t*>(Mem_Calloc(fs_mempool, sizeof(search_t) + numchars + numfiles * sizeof(char*)));
...
static const wadtype_t wad_types[7] = {{"pal", TYP_PALETTE}, // palette
...
wfile_t* W_Open(const char* filename, int* error)
{
	...
	if (fs_ext_path)
		...
		basename = COM_FileWithoutPath(filename);
...
public:
	...
	virtual bool FileExists(const char* file, bool casesensitive = false) { return (bool)FS_FileExists(file, casesensitive); }

	virtual void AddSearchPath(const char* dir) { FS_AddGameDirectory(dir, FS_CUSTOM_PATH); }
