...
static qboolean HPAK_Validate(const char* filename, qboolean quiet)
{
	...
	for (i = 0; i < num_lumps; i++)
	{
		...
		Con_Printf("%i:      %s %s %s:   ", i, HPAK_TypeFromIndex(pRes->type), Q_pretifymem(pRes->nDownloadSize, 2), pRes->szFileName);
...
void HPAK_ValidatePak(const char* filename) { HPAK_Validate(filename, true); }
