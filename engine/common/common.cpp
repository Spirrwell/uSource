...
static const char* file_exts[] = {
	"cfg", "lst", "exe", "vbs", "com", "bat", "dll", "ini", "log", "sys",
};
...
=============
*/
int COM_FileSize(const char* filename) { return FS_FileSize(filename, false); }
...
=============
*/
void COM_AddAppDirectoryToSearchPath(const char* pszBaseDir, const char* appName) { FS_AddGameHierarchy(pszBaseDir, FS_NOWRITE_PATH); }
...
=============
*/
byte* COM_LoadFile(const char* filename, int usehunk, int* pLength) { return COM_LoadFileForMe(filename, pLength); }
...
=============
*/
void COM_FreeFile(void* buffer) { free(buffer); }
...
=============
*/
void pfnCvar_RegisterEngineVariable(cvar_t* variable) { Cvar_RegisterVariable((convar_t*)variable); }
...
=============
*/
cvar_t* pfnCVarGetPointer(const char* szVarName) { return (cvar_t*)Cvar_FindVar(szVarName); }
...
=============
*/
void pfnCVarDirectSet(cvar_t* var, const char* szValue) { Cvar_DirectSet((convar_t*)var, szValue); }
...
=============
*/
float pfnTime(void) { return (float)Sys_DoubleTime(); }
...
=============
*/
int GAME_EXPORT GAME_EXPORT pfnIsCareerMatch(void) { return 0; }
...
=============
*/
int GAME_EXPORT pfnGetTimesTutorMessageShown(int mid) { return 0; }
...
public:
	...
	virtual void MemCheck() { Mem_Check(); }

	virtual void MemPrintStats() { Mem_PrintStats(); }

	virtual void ClientShutdown() { CL_Shutdown(); }

	virtual void ServerShutdown() { SV_Shutdown(""); }

	virtual void HostError(const char* fmt) { Host_Error("%s", fmt); }
};
