...
qboolean NetSplit_GetLong(netsplit_t* ns, netadr_t* from, byte* data, size_t* length)
{
	...
	if (p->received == packet->count)
	{
		...
		*length = len;
		...
		// (int)p->received, (int)packet->length );
...
void Netchan_Shutdown(void) { Mem_FreePool(&net_mempool); }
...
void Netchan_TransmitBits(netchan_t* chan, int length, byte* data)
{
	...
	{
		Con_Printf(" %s --> sz=%i seq=%i ack=%i rel=%i tm=%f\n", ns_strings[chan->sock], MSG_GetNumBytesWritten(&send),
			   (chan->outgoing_sequence - 1) & 63, chan->incoming_sequence & 63, send_reliable ? 1 : 0, (float)host.realtime);
	}
}
...
================
*/
void Netchan_Transmit(netchan_t* chan, int lengthInBytes, byte* data) { Netchan_TransmitBits(chan, lengthInBytes << 3, data); }
