...
public:
	...
	virtual void	     RegisterCvar(const char* name, const char* default_val, const char* desc, int flags,
					  void (*callback)(const char*, const char*))			  = 0;
...
public:
	...
	virtual void		  ClientPrintf(edict_t* pEdict, PRINT_TYPE ptype,
					       const char* szMsg) = 0; // JOHN: engine callbacks so game DLL can print messages to individual clients
	...
	virtual void		  RunPlayerMove(edict_t* fakeclient, const float* viewangles, float forwardmove, float sidemove, float upmove,
						unsigned short buttons, byte impulse, byte msec) = 0;
	...
	virtual int GetPlayerUserId(edict_t* e) = 0; // returns the server assigned userid for this player.  useful for logging frags, etc.  returns
						     // -1 if the edict couldn't be found in the list of clients
	virtual void BuildSoundMsg(edict_t* entity, int channel, const char* sample, /*int*/ float volume, float attenuation, int fFlags, int pitch,
				   int msg_dest, int msg_type, const float* pOrigin, edict_t* ed) = 0;
	virtual int  IsDedicatedServer(void)							  = 0; // is this a dedicated server?
	virtual unsigned int GetPlayerWONId(edict_t* e) = 0; // returns the server assigned WONid for this player.  useful for logging frags, etc.
	...
	virtual void	       PlaybackEvent(int flags, const edict_t* pInvoker, unsigned short eventindex, float delay, float* origin, float* angles,
	...
	virtual void	       DeltaAddEncoder(char* name,
					       void (*conditionalencode)(struct delta_s* pFields, const unsigned char* from, const unsigned char* to)) = 0;
