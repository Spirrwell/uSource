...
const char* clc_strings[clc_lastmsg + 1] = {
	"clc_bad",	 "clc_nop",	  "clc_move",	    "clc_stringcmd", "clc_delta", "clc_resourcelist", "clc_unused6", "clc_fileconsistency",
	"clc_voicedata", "clc_cvarvalue", "clc_cvarvalue2",
};
...
================
*/
void SV_Ack(netadr_t from) { Con_Printf("ping %s\n", NET_AdrToString(from)); }
...
================
*/
void SV_Ping(netadr_t from) { Netchan_OutOfBandPrint(NS_SERVER, from, "ack"); }
...
{
	SV_ClientPrintf(cl, "Server running %s %s (build %i-%s, %s-%s)\n", XASH_ENGINE_NAME, XASH_VERSION, Q_buildnum(), Q_buildcommit(), Q_buildos(),
			Q_buildarch());
	return true;
}
...
void pfnHookServerNetsystemMsg(void (*pfnHook)(edict_t*, void*)) { g_sv_msg_hooks[clc_netsystem].push_back(pfnHook); }
