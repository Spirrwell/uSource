...
int SV_FlyMove(edict_t* ent, float time, trace_t* steptrace)
{
	...
	for (bumpcount = 0; bumpcount < MAX_CLIP_PLANES - 1; bumpcount++)
	{
		...
		if (trace.plane.normal[2] > 0.7f)
		{
			...
			if (trace.ent->v.solid == SOLID_BSP || trace.ent->v.solid == SOLID_SLIDEBOX || trace.ent->v.movetype == MOVETYPE_PUSHSTEP ||
			    (trace.ent->v.flags & FL_CLIENT))
			{
...
void SV_Physics_Pusher(edict_t* ent)
{
	...
	if (pBlocker)
		g_pServerInterface->Blocked(ent, pBlocker);
...
=============
*/
void SV_Physics_None(edict_t* ent) { SV_RunThink(ent); }
...
================
*/
double SV_GetServerTime(void) { return sv.time; }
...
================
*/
double SV_GetFrameTime(void) { return sv.frametime; }
...
================
*/
areanode_t* SV_GetHeadNode(void) { return sv_areanodes; }
...
================
*/
int SV_ServerState(void) { return sv.state; }
...
static void* pfnMem_Alloc(size_t cb, const char* filename, const int fileline) { return _Mem_Alloc(svgame.mempool, cb, true, filename, fileline); }
