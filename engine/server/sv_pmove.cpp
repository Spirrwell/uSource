...
static int pfnTestPlayerPosition(float* pos, pmtrace_t* ptrace) { return PM_TestPlayerPosition(svgame.pmove, pos, ptrace, NULL); }
...
static int pfnTruePointContents(float* p) { return PM_TruePointContents(svgame.pmove, p); }
...
static hull_t* pfnHullForBsp(physent_t* pe, float* offset) { return PM_HullForBsp(pe, svgame.pmove, offset); }
...
static void pfnPlaybackEventFull(int flags, int clientindex, word eventindex, float delay, float* origin, float* angles, float fparam1, float fparam2,
				 int iparam1, int iparam2, int bparam1, int bparam2)
{
	...
	SV_PlaybackEventFull(flags, ent, eventindex, delay, origin, angles, fparam1, fparam2, iparam1, iparam2, bparam1, bparam2);
}
...
void SV_InitClientMove(void)
{
	...
	for (i = 0; i < MAX_MAP_HULLS; i++)
	{
		if (g_pServerInterface->GetHullBounds(i, host.player_mins[i], host.player_maxs[i]))
			Con_Reportf("SV: hull%i, player_mins: %g %g %g, player_maxs: %g %g %g\n", i, host.player_mins[i][0], host.player_mins[i][1],
				    host.player_mins[i][2], host.player_maxs[i][0], host.player_maxs[i][1], host.player_maxs[i][2]);
	}
