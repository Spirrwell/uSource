...
hull_t* SV_HullForStudioModel(edict_t* ent, vec3_t mins, vec3_t maxs, vec3_t offset, int* numhitboxes)
{
	...
	if (FBitSet(mod->flags, STUDIO_TRACE_HITBOX) || useComplexHull)
	{
		...
		if (FBitSet(ent->v.flags, FL_CLIENT | FL_FAKECLIENT))
		{
			...
			hull = Mod_HullForStudio(mod, ent->v.frame, ent->v.sequence, angles, ent->v.origin, size, controller, blending, numhitboxes,
						 ent);
		}
		else
		{
			hull = Mod_HullForStudio(mod, ent->v.frame, ent->v.sequence, ent->v.angles, ent->v.origin, size, ent->v.controller,
						 ent->v.blending, numhitboxes, ent);
		}
	}
