...
static TYPEDESCRIPTION gEntvarsDescription[] = {
	DEFINE_ENTITY_FIELD(classname, FIELD_STRING),	   DEFINE_ENTITY_FIELD(globalname, FIELD_STRING),
	DEFINE_ENTITY_FIELD(model, FIELD_MODELNAME),	   DEFINE_ENTITY_FIELD(viewmodel, FIELD_MODELNAME),
	DEFINE_ENTITY_FIELD(weaponmodel, FIELD_MODELNAME), DEFINE_ENTITY_FIELD(target, FIELD_STRING),
	DEFINE_ENTITY_FIELD(targetname, FIELD_STRING),	   DEFINE_ENTITY_FIELD(netname, FIELD_STRING),
	DEFINE_ENTITY_FIELD(message, FIELD_STRING),	   DEFINE_ENTITY_FIELD(noise, FIELD_SOUNDNAME),
	DEFINE_ENTITY_FIELD(noise1, FIELD_SOUNDNAME),	   DEFINE_ENTITY_FIELD(noise2, FIELD_SOUNDNAME),
...
=============
*/
char* SV_Serverinfo(void) { return svs.serverinfo; }
...
=============
*/
char* SV_Localinfo(void) { return svs.localinfo; }
...
=======================
*/
sizebuf_t* SV_GetReliableDatagram(void) { return &sv.reliable_datagram; }
...
void pfnSetModel(edict_t* e, const char* m)
{
	...
	if (mod && mod->type != mod_studio)
		...
		SV_SetMinMaxSize(e, vec3_origin, vec3_origin, true);
}
...
=================
*/
float pfnVecToYaw(const float* rgflVector) { return SV_VecToYaw(rgflVector); }
...
==============
*/
void pfnMakeVectors(const float* rgflVector) { AngleVectors(rgflVector, svgame.globals->v_forward, svgame.globals->v_right, svgame.globals->v_up); }
...
==============
*/
edict_t* pfnCreateNamedEntity(string_t className) { return SV_CreateNamedEntity(NULL, className); }
...
static void pfnTraceHull(const float* v1, const float* v2, int fNoMonsters, int hullNumber, edict_t* pentToSkip, TraceResult* ptr)
{
	...
	trace = SV_Move(v1, sv.worldmodel->hulls[hullNumber].clip_mins, sv.worldmodel->hulls[hullNumber].clip_maxs, v2, fNoMonsters, pentToSkip,
			false);
	SV_ConvertTrace(ptr, &trace);
}
...
=============
*/
void pfnBuildSoundMsg(edict_t* pSource, int chan, const char* samp, float fvol, float attn, int fFlags, int pitch, int msg_dest, int msg_type,
		      const float* pOrigin, edict_t* pSend)
{
...
=============
*/
edict_t* pfnPEntityOfEntOffset(int iEntOffset) { return (edict_t*)((byte*)svgame.edicts + iEntOffset); }
...
=============
*/
int pfnEntOffsetOfPEntity(const edict_t* pEdict) { return (byte*)pEdict - (byte*)svgame.edicts; }
...
=============
*/
void* pfnFunctionFromName(const char* pName) { return COM_FunctionFromName_SR(svgame.hInstance, pName); }
...
=============
*/
const char* pfnNameForFunction(void* function) { return COM_NameForFunction(svgame.hInstance, function); }
...
=============
*/
int pfnIsDedicatedServer(void) { return Host_IsDedicated(); }
...
=============
*/
uint pfnGetPlayerWONId(edict_t* e) { return (uint)-1; }
...
=============
*/
word pfnPrecacheEvent(int type, const char* psz) { return (word)SV_EventIndex(psz); }
...
=============
*/
void SV_PlaybackEventFull(int flags, const edict_t* pInvoker, word eventindex, float delay, float* origin, float* angles, float fparam1,
...
=============
*/
const char* pfnGetPlayerAuthId(edict_t* e) { return SV_GetClientIDString(SV_ClientFromEdict(e, false)); }
...
qboolean SV_LoadProgs(const char* name)
{
	...
	AppFramework::SetLoadLibrary([](const char* lib) -> void* { return COM_LoadLibrary(lib, 0, false); });
	AppFramework::SetFreeLibrary([](void* lib) { COM_FreeLibrary(lib); });
