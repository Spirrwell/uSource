...
void* COM_LoadLibrary(const char* dllname, int build_ordinals_table) { return LoadLibraryA(dllname); }

void COM_FreeLibrary(void* hInstance) { FreeLibrary(hInstance); }

void* COM_GetProcAddress(void* hInstance, const char* name) { return GetProcAddress(hInstance, name); }

void* COM_FunctionFromName(void* hInstance, const char* name) { return GetProcAddress(hInstance, name); }
...
static void CopySections(const byte* data, PIMAGE_NT_HEADERS old_headers, PMEMORYMODULE module)
{
	...
	for (i = 0; i < module->headers->FileHeader.NumberOfSections; i++, section++)
	{
		if (section->SizeOfRawData == 0)
		{
			...
			if (size > 0)
			{
				dest = (byte*)VirtualAlloc((byte*)CALCULATE_ADDRESS(codeBase, section->VirtualAddress), size, MEM_COMMIT,
							   PAGE_READWRITE);
		...
		dest = (byte*)VirtualAlloc((byte*)CALCULATE_ADDRESS(codeBase, section->VirtualAddress), section->SizeOfRawData, MEM_COMMIT,
					   PAGE_READWRITE);
...
void* MemoryLoadLibrary(const char* name)
{
	...
	code = (byte*)VirtualAlloc((LPVOID)(old_header->OptionalHeader.ImageBase), old_header->OptionalHeader.SizeOfImage, MEM_RESERVE,
				   PAGE_READWRITE);
