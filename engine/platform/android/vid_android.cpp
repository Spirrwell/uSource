...
========================
*/
static void Android_SwapInterval(int interval) { eglSwapInterval(negl.dpy, interval); }
...
========================
*/
static void Android_GetScreenRes(int* width, int* height) { *width = jni.width, *height = jni.height; }
...
========================
*/
void GL_SwapBuffers() { eglSwapBuffers(negl.dpy, negl.surface); }
...
static const EGLint* VID_GenerateConfig(void)
{
	...
	if (gl_attribs_set[REF_GL_CONTEXT_PROFILE_MASK] && !(gl_attribs[REF_GL_CONTEXT_PROFILE_MASK] & REF_GL_CONTEXT_PROFILE_ES))
	{
		attribs[i++] = EGL_OPENGL_BIT;
		gl_api	     = EGL_OPENGL_API;
	}
	else if (gl_attribs_set[REF_GL_CONTEXT_MAJOR_VERSION] && gl_attribs[REF_GL_CONTEXT_MAJOR_VERSION] >= 2)
	{
...
void* SW_LockBuffer() { return NULL; }
