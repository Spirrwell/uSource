...
//#define VectorDistance2(a, b) (((a)[0] - (b)[0]) * ((a)[0] - (b)[0]) + ((a)[1] - (b)[1]) * ((a)[1] - (b)[1]) + ((a)[2] - (b)[2]) * ((a)[2] -
//(b)[2]))
...
//#define VectorLerp( v1, lerp, v2, c ) ((c)[0] = (v1)[0] + (lerp) * ((v2)[0] - (v1)[0]), (c)[1] = (v1)[1] + (lerp) * ((v2)[1] - (v1)[1]), (c)[2] =
//(v1)[2] + (lerp) * ((v2)[2] - (v1)[2]))

inline void VectorNormalize(Vector3& vec) { vec = vec.Normalize(); };
//#define VectorNormalize( v ) { float ilength = (float)sqrt(DotProduct(v, v));if (ilength) ilength = 1.0f / ilength;v[0] *= ilength;v[1] *=
//ilength;v[2] *= ilength; }

inline void VectorNormalize2(const Vector3& vec, Vector3& dest) { dest = vec.Normalize(); };
//#define VectorNormalize2( v, dest ) {float ilength = (float)sqrt(DotProduct(v,v));if (ilength) ilength = 1.0f / ilength;dest[0] = v[0] *
//ilength;dest[1] = v[1] * ilength;dest[2] = v[2] * ilength; }
