...
void CL_ParseNetsysMessage(void* msg)
{
	...
	case HDR_USRMSG:
	{
		...
		{
			if (callback.msgid != usermsghdr.id)
				continue;
...
//=========================================================================================================================//
// CNetworkMessage
CNetworkMessage::CNetworkMessage(unsigned long size) : m_size(size), m_init(false), m_data(nullptr), m_bufpos(0), m_overflowed(false) {}

CNetworkMessage::CNetworkMessage(void* data, size_t size) : m_init(true), m_data(nullptr), m_size(size), m_bufpos(0), m_overflowed(false)
{
	Assert(size > 0);
...
#define WRITE_TO_BUFFER(x)                                                                                                                           \
	if (m_bufpos + sizeof(x) >= m_size)                                                                                                          \
	{                                                                                                                                            \
		m_overflowed = true;                                                                                                                 \
		return;                                                                                                                              \
	};                                                                                                                                           \
	auto y = ToLittleEndian(x);                                                                                                                  \
	memcpy((void*)((uintptr_t)m_data + m_bufpos), &y, sizeof(y));                                                                                \
	m_bufpos += sizeof(y);

void CNetworkMessage::WriteInt64(long long int l) { WRITE_TO_BUFFER(l); }

void CNetworkMessage::WriteUInt64(unsigned long long int l) { WRITE_TO_BUFFER(l); }

void CNetworkMessage::WriteInt(int i) { WRITE_TO_BUFFER(i); }

void CNetworkMessage::WriteUInt(unsigned int i) { WRITE_TO_BUFFER(i); }

void CNetworkMessage::WriteShort(short s) { WRITE_TO_BUFFER(s); }

void CNetworkMessage::WriteUShort(unsigned short s) { WRITE_TO_BUFFER(s); }

void CNetworkMessage::WriteByte(unsigned char b) { WRITE_TO_BUFFER(b); }

void CNetworkMessage::WriteChar(char c) { WRITE_TO_BUFFER(c); }
...
void CNetworkMessage::Resize(unsigned long newsize) { m_data = Mem_Realloc(m_data, newsize); }

void CNetworkMessage::Seek(unsigned int type, unsigned long pos)
{
	if (type == CNetworkMessage::ABSOLUTE)
		m_bufpos = pos;
	else if (type == CNetworkMessage::RELATIVE)
		m_bufpos += pos;
}
