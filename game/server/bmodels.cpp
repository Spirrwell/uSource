...
//
Vector VecBModelOrigin(entvars_t* pevBModel) { return pevBModel->absmin + (pevBModel->size * 0.5); }
...
public:
	...
	virtual void Touch(CBaseEntity* toucher) override;
...
TYPEDESCRIPTION CFuncRotating::m_SaveData[] = {
	DEFINE_FIELD(CFuncRotating, m_flFanFriction, FIELD_FLOAT), DEFINE_FIELD(CFuncRotating, m_flAttenuation, FIELD_FLOAT),
	DEFINE_FIELD(CFuncRotating, m_flVolume, FIELD_FLOAT), DEFINE_FIELD(CFuncRotating, m_pitch, FIELD_FLOAT),
...
CFuncRotating::CFuncRotating() : m_bSpinDown(false), m_bSpinUp(false), m_bEnableHurt(false), m_bDelayUse(false), m_bRotate(false) {}
...
void CFuncRotating::SpinUp(void)
{
	    ...
	    fabs(vecAVel.z) >= fabs(pev->movedir.z * pev->speed))
	{
		...
		EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, STRING(pev->noiseRunning), m_flVolume, m_flAttenuation, SND_CHANGE_PITCH | SND_CHANGE_VOL,
			       FANPITCHMAX);
...
void CFuncRotating::Rotate(void) { pev->nextthink = pev->ltime + 10; }
...
//
void CFuncRotating::Blocked(CBaseEntity* pOther) { pOther->TakeDamage(pev, pev, pev->dmg, DMG_CRUSH); }

void CFuncRotating::Touch(CBaseEntity* toucher)
{
	CBaseEntity::Touch(toucher);

	if (m_bEnableHurt)
		HurtTouch(toucher);
}

void CFuncRotating::Think()
{
	CBaseEntity::Think();
	if (m_bSpinUp)
		this->SpinUp();
...
TYPEDESCRIPTION CPendulum::m_SaveData[] = {
	DEFINE_FIELD(CPendulum, m_accel, FIELD_FLOAT),	  DEFINE_FIELD(CPendulum, m_distance, FIELD_FLOAT),
	DEFINE_FIELD(CPendulum, m_time, FIELD_TIME),	  DEFINE_FIELD(CPendulum, m_damp, FIELD_FLOAT),
	DEFINE_FIELD(CPendulum, m_maxSpeed, FIELD_FLOAT), DEFINE_FIELD(CPendulum, m_dampSpeed, FIELD_FLOAT),
	DEFINE_FIELD(CPendulum, m_center, FIELD_VECTOR),  DEFINE_FIELD(CPendulum, m_start, FIELD_VECTOR),
};

IMPLEMENT_SAVERESTORE(CPendulum, CBaseEntity)

CPendulum::CPendulum() : m_bDelayUse(false), m_bSwinging(false), m_bStop(false), m_bAllowTouch(false) {}
...
void CPendulum::Blocked(CBaseEntity* pOther) { m_time = gpGlobals->time; }
