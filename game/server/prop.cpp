...
const char* CProp::pSoundsFlesh[] = {
	"debris/flesh1.wav", "debris/flesh2.wav", "debris/flesh3.wav", "debris/flesh5.wav", "debris/flesh6.wav", "debris/flesh7.wav",
};
...
void CProp::BounceTouch(CBaseEntity* pOther)
{
	...
	if (pev->flags & FL_ONGROUND || fabs(pev->velocity.z) < 40)
	{
		...
		else if (m_shape == SHAPE_SPHERE)
		{
			pev->velocity.z -= 20;
			pev->avelocity.x =
				-cos(M_PI / 180 * UTIL_AngleDiff(UTIL_VecToAngles(pev->velocity).y, pev->angles.y)) * pev->velocity.Length() * 1.5;
			pev->avelocity.y =
				-sin(M_PI / 180 * UTIL_AngleDiff(UTIL_VecToAngles(pev->velocity).y, pev->angles.y)) * pev->velocity.Length() * 1.5;
			;
			pev->avelocity.z = 0;
		}
...
void CProp::AngleThink()
{
	...
	else if (m_shape == SHAPE_NOROTATE)
	{
		...
		if (fabs(UTIL_AngleDiff(iangles.x, pev->angles.x)) > 0.1 || fabs(UTIL_AngleDiff(iangles.y, pev->angles.y)) > 0.1 ||
		    fabs(UTIL_AngleDiff(iangles.z, pev->angles.z)) > 0.1)
		{
			SetThink(&CProp::AngleThink);
			pev->nextthink = gpGlobals->time + 0.1;
	...
	else if (m_shape == SHAPE_GENERIC)
	{
		...
		if (fabs(UTIL_AngleDiff(ianglex, pev->angles.x)) > 0.1 || fabs(UTIL_AngleDiff(ianglez, pev->angles.z)) > 0.1)
		{
			SetThink(&CProp::AngleThink);
			pev->nextthink = gpGlobals->time + 0.1;
		}
