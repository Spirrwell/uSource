...
TYPEDESCRIPTION CController::m_SaveData[] = {
	DEFINE_ARRAY(CController, m_pBall, FIELD_CLASSPTR, 2),	   DEFINE_ARRAY(CController, m_iBall, FIELD_INTEGER, 2),
	DEFINE_ARRAY(CController, m_iBallTime, FIELD_TIME, 2),	   DEFINE_ARRAY(CController, m_iBallCurrent, FIELD_INTEGER, 2),
...
const char* CController::pIdleSounds[] = {
	"controller/con_idle1.wav", "controller/con_idle2.wav", "controller/con_idle3.wav", "controller/con_idle4.wav", "controller/con_idle5.wav",
};
...
// relationship table.
//=========================================================
int CController::Classify(void) { return CLASS_ALIEN_MILITARY; }
...
void CController::AlertSound(void) { EMIT_SOUND_ARRAY_DYN(CHAN_VOICE, pAlertSounds); }
...
Schedule_t slControllerChaseEnemy[] = {
	{tlControllerChaseEnemy, ARRAYSIZE(tlControllerChaseEnemy), bits_COND_NEW_ENEMY | bits_COND_TASK_FAILED, 0, "ControllerChaseEnemy"},
};
...
BOOL CController::CheckMeleeAttack1(float flDot, float flDist) { return FALSE; }
...
void CController::Stop(void) { m_IdealActivity = GetStoppedActivity(); }
...
void CControllerHeadBall::HuntThink(void)
{
	...
	if (gpGlobals->time - pev->dmgtime > 5 || pev->renderamt < 64 || m_hEnemy == 0 || m_hOwner == 0 || pev->origin.x < -4096 ||
	    pev->origin.x > 4096 || pev->origin.y < -4096 || pev->origin.y > 4096 || pev->origin.z < -4096 || pev->origin.z > 4096)
	{
		SetTouch(NULL);
		UTIL_Remove(this);
		return;
