...
public:
	...
	bool m_bTouchEnable : 1;
	bool m_bDoorDown : 1;
	bool m_bDoorGoingDown : 1;
	bool m_bDoorGoingUp : 1;
};

TYPEDESCRIPTION CBaseDoor::m_SaveData[] = {
	DEFINE_FIELD(CBaseDoor, m_bHealthValue, FIELD_CHARACTER),   DEFINE_FIELD(CBaseDoor, m_bMoveSnd, FIELD_CHARACTER),
	DEFINE_FIELD(CBaseDoor, m_bStopSnd, FIELD_CHARACTER),

	DEFINE_FIELD(CBaseDoor, m_bLockedSound, FIELD_CHARACTER),   DEFINE_FIELD(CBaseDoor, m_bLockedSentence, FIELD_CHARACTER),
	DEFINE_FIELD(CBaseDoor, m_bUnlockedSound, FIELD_CHARACTER), DEFINE_FIELD(CBaseDoor, m_bUnlockedSentence, FIELD_CHARACTER),
};
...
void PlayLockSounds(entvars_t* pev, locksound_t* pls, int flocked, int fbutton)
{
	...
	if (flocked)
	{
		...
		if (fplaysentence)
		{
			...
			pls->iLockedSentence   = SENTENCEG_PlaySequentialSz(ENT(pev), STRING(pls->sLockedSentence), 0.85, ATTN_NORM, 0, 100,
									    pls->iLockedSentence, FALSE);
	...
	else
	{
		...
		if (fplaysentence)
		{
			...
			pls->iUnlockedSentence = SENTENCEG_PlaySequentialSz(ENT(pev), STRING(pls->sUnlockedSentence), 0.85, ATTN_NORM, 0, 100,
									    pls->iUnlockedSentence, FALSE);
...
CBaseDoor::CBaseDoor() : m_bDoorDown(false), m_bTouchEnable(false), m_bDoorGoingDown(false), m_bDoorGoingUp(false) {}
...
void CBaseDoor::Spawn()
{
	...
	m_vecPosition2 = m_vecPosition1 + (pev->movedir * (fabs(pev->movedir.x * (pev->size.x - 2)) + fabs(pev->movedir.y * (pev->size.y - 2)) +
...
{
	CBaseEntity::Touch(other);
	if (m_bTouchEnable)
		this->DoorTouch(other);
}
...
public:
	...
	virtual void Spawn(void) override;
	virtual void Precache(void) override;
	virtual void MoveDone(void) override;
...
CMomentaryDoor::CMomentaryDoor() : m_bTouchEnable(false), m_bStopSound(false) {}
