...
void CApache::HuntThink(void)
{
	...
	if (flLength > 250) // 500
	{
		// float flLength2 = ( m_posTarget - pev->origin ).Length() * ( 1.5 - DotProduct( ( m_posTarget - pev->origin ).Normalize(),
		// pev->velocity.Normalize() ) ); if( flLength2 < flLength )
	...
	// ALERT( at_console, "%d %d %d %4.2f\n", pev->angles.x < 0, DotProduct( pev->velocity, gpGlobals->v_forward ) > -100, m_flNextRocket <
	// gpGlobals->time, DotProduct( m_vecTarget, vecEst ) );
...
void CApache::Flight(void)
{
	...
	// ALERT( at_console, "%.0f %.0f : %.0f %.0f : %.0f %.0f : %.0f\n", pev->origin.x, pev->velocity.x, flDist, flSpeed, pev->angles.x,
	// pev->avelocity.x, m_flForce ); ALERT( at_console, "%.0f %.0f : %.0f %0.f : %.0f\n", pev->origin.z, pev->velocity.z, vecEst.z,
	// m_posDesired.z, m_flForce );
...
void CApacheHVR::AccelerateThink(void)
{
	...
	if (pev->origin.x < -4096 || pev->origin.x > 4096 || pev->origin.y < -4096 || pev->origin.y > 4096 || pev->origin.z < -4096 ||
	    pev->origin.z > 4096)
	{
		UTIL_Remove(this);
		return;
