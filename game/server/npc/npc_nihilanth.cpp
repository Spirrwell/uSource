...
TYPEDESCRIPTION CNihilanthHVR::m_SaveData[] = {
	DEFINE_FIELD(CNihilanthHVR, m_flIdealVel, FIELD_FLOAT),	   DEFINE_FIELD(CNihilanthHVR, m_vecIdeal, FIELD_VECTOR),
	DEFINE_FIELD(CNihilanthHVR, m_pNihilanth, FIELD_CLASSPTR), DEFINE_FIELD(CNihilanthHVR, m_hTouch, FIELD_EHANDLE),
...
void CNihilanth::DeathSound(void) { EMIT_SOUND(edict(), CHAN_VOICE, RANDOM_SOUND_ARRAY(pDeathSounds), 1.0, 0.1); }
...
void CNihilanth::Killed(entvars_t* pevAttacker, int iGib) { CBaseMonster::Killed(pevAttacker, iGib); }
		...
		if (m_hFriend[i] == 0)
		{
			if (RANDOM_LONG(0, 1) == 0)
			{
				...
				if (iNode != NO_NODE)
				{
					...
					TraceResult tr;
					UTIL_TraceHull(node.m_vecOrigin + Vector(0, 0, 32), node.m_vecOrigin + Vector(0, 0, 32), dont_ignore_monsters,
						       large_hull, NULL, &tr);
					if (tr.fStartSolid == 0)
			...
			else
			{
				...
				if (iNode != NO_NODE)
				{
					CNode&	    node = WorldGraph.Node(iNode);
					TraceResult tr;
					UTIL_TraceHull(node.m_vecOrigin + Vector(0, 0, 36), node.m_vecOrigin + Vector(0, 0, 36), dont_ignore_monsters,
						       human_hull, NULL, &tr);
					if (tr.fStartSolid == 0)
...
void CNihilanthHVR::ZapThink(void)
{
	...
	if (m_hEnemy == 0 || pev->origin.x < -4096 || pev->origin.x > 4096 || pev->origin.y < -4096 || pev->origin.y > 4096 ||
...
void CNihilanthHVR::TeleportThink(void)
{
	...
	// check world boundaries
	if (m_hEnemy == 0 || !m_hEnemy->IsAlive() || pev->origin.x < -4096 || pev->origin.x > 4096 || pev->origin.y < -4096 || pev->origin.y > 4096 ||
	    pev->origin.z < -4096 || pev->origin.z > 4096)
	{
