...
TYPEDESCRIPTION CBasePlayer::m_playerSaveData[] = {
	...
	// DEFINE_FIELD( CBasePlayer, m_nCustomSprayFrames, FIELD_INTEGER ), // Don't restore, depends on server message after spawning and only
	// matters in multiplayer DEFINE_FIELD( CBasePlayer, m_vecAutoAim, FIELD_VECTOR ), // Don't save/restore - this is recomputed DEFINE_ARRAY(
	// CBasePlayer, m_rgAmmoLast, FIELD_INTEGER, MAX_AMMO_SLOTS ), // Don't need to restore DEFINE_FIELD( CBasePlayer, m_fOnTarget, FIELD_BOOLEAN
	// ), // Don't need to restore DEFINE_FIELD( CBasePlayer, m_nCustomSprayFrames, FIELD_INTEGER ), // Don't need to restore
};
...
int CBasePlayer::TakeHealth(float flHealth, int bitsDamageType) { return CBaseMonster::TakeHealth(flHealth, bitsDamageType); }
...
BOOL CBasePlayer::IsOnLadder(void) { return (pev->movetype == MOVETYPE_FLY); }
...
void CBasePlayer::PlayerDeathThink(void)
{
	...
	// once we're done animating our death and we're on the ground, we want to set movetype to None so our dead body won't do collisions and stuff
	// anymore this prevents a bug where the dead body would go to a player's head if he walked over it while the dead player was clicking their
	// button to respawn
...
// ID's player as such.
//
int CBasePlayer::Classify(void) { return CLASS_PLAYER; }
...
CBaseEntity* FindEntityForward(CBaseEntity* pMe)
{
	...
	UTIL_TraceLine(pMe->pev->origin + pMe->pev->view_ofs, pMe->pev->origin + pMe->pev->view_ofs + gpGlobals->v_forward * 8192,
		       dont_ignore_monsters, pMe->edict(), &tr);
...
BOOL CBasePlayer::FlashlightIsOn(void) { return FBitSet(pev->effects, EF_DIMLIGHT); }
...
int CBasePlayer::RemovePlayerItem(CBasePlayerItem* pItem, bool bCallHolster)
{
	...
	// In some cases an item can be both the active and last item, like for instance dropping all weapons and only having an exhaustible weapon
	// left. - Solokiller
...
//=========================================================
void CBasePlayer::BarnacleVictimReleased(void) { m_afPhysicsFlags &= ~PFLAG_ONBARNACLE; }
...
=============
*/
int CBasePlayer::GetCustomDecalFrames(void) { return m_nCustomSprayFrames; }
...
void CBasePlayer::DropPlayerItem(char* pszItemName)
{
	...
	for (i = 0; i < MAX_ITEM_TYPES; i++)
	{
		...
		if (pWeapon)
		{
			...
			CWeaponBox* pWeaponBox =
				(CWeaponBox*)CBaseEntity::Create("weaponbox", pev->origin + gpGlobals->v_forward * 10, pev->angles, edict());
