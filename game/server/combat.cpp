	...
	for (cSplat = 0; cSplat < cGibs; cSplat++)
	{
		...
		else
		{
			if (human)
			{
				...
				pGib->pev->body =
					RANDOM_LONG(1, HUMAN_GIB_COUNT - 1); // start at one to avoid throwing random amounts of skulls (0th gib)
			}
			else
			{
				// aliens
...
BOOL CBaseMonster::HasAlienGibs(void)
{
	...
	if (myClass == CLASS_ALIEN_MILITARY || myClass == CLASS_ALIEN_MONSTER || myClass == CLASS_ALIEN_PASSIVE || myClass == CLASS_INSECT ||
	    myClass == CLASS_ALIEN_PREDATOR || myClass == CLASS_ALIEN_PREY)
...
int CBaseMonster::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
	...
	// grab the vector of the incoming attack. ( pretend that the inflictor is a little lower than it really is, so the body will tend to fly
	// upward a bit).
	vecDir = Vector(0, 0, 0);
...
void RadiusDamage(Vector vecSrc, entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, float flRadius, int iClassIgnore,
		  int bitsDamageType)
{
