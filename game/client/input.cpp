...
============
*/
int DLLEXPORT HUD_Key_Event(int down, int keynum, const char* pszCurrentBinding) { return 1; }
...
void IN_Impulse(void) { in_impulse = atoi(gEngfuncs.Cmd_Argv(1)); }
...
============
*/
int CL_IsDead(void) { return (gHUD.m_Health.m_iHealth <= 0) ? 1 : 0; }
...
void DLLEXPORT HUD_Shutdown(void) { ShutdownInput(); }
...
public:
	...
	virtual void InitInput() { IN_Init(); }

	virtual void ShutdownInput() { ::ShutdownInput(); }

	virtual void ActivateMouse(void) { currentInput->IN_ActivateMouse(); }

	virtual void DeactivateMouse(void) { currentInput->IN_DeactivateMouse(); }

	virtual void MouseEvent(int mstate) { currentInput->IN_MouseEvent(mstate); }

	virtual void ClearStates(void) { currentInput->IN_ClearStates(); }

	virtual void Accumulate(void) { currentInput->IN_Accumulate(); }

	virtual int TouchEvent(int type, int fingerID, float x, float y, float dx, float dy) { return 0; }

	virtual void MoveEvent(float forwardmove, float sidemove) { currentInput->IN_ClientMoveEvent(forwardmove, sidemove); }

	virtual void LookEvent(float relyaw, float relpitch) { currentInput->IN_ClientLookEvent(relyaw, relpitch); }

	virtual void* KB_Find(const char* name) { return ::KB_Find(name); }

	virtual int KeyEvent(int evcode, int keynum, const char* bind) { return HUD_Key_Event(evcode, keynum, bind); }
};
