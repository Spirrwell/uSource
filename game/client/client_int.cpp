...
extern "C"
{
	...
	extern void DLLEXPORT	  HUD_TxferPredictionData(struct entity_state_s* ps, const struct entity_state_s* pps, struct clientdata_s* pcd,
							  const struct clientdata_s* ppcd, struct weapon_data_s* wd, const struct weapon_data_s* pwd);
	extern void DLLEXPORT	  HUD_TempEntUpdate(double frametime, double client_time, double cl_gravity, struct tempent_s** ppTempEntFree,
...
public:
	...
	virtual void InitHud() { HUD_Init(); }

	virtual void Shutdown(){};

	virtual int Initialize(cl_enginefunc_t* pEnginefuncs, int iVersion) { return ::Initialize(pEnginefuncs, iVersion); }

	virtual int VidInit(void) { return HUD_VidInit(); }

	virtual int Redraw(float flTime, int intermission) { return HUD_Redraw(flTime, intermission); }

	virtual int UpdateClientData(client_data_t* cdata, float flTime) { return HUD_UpdateClientData(cdata, flTime); }

	virtual void Reset(void) { HUD_Reset(); }

	virtual void PlayerMove(struct playermove_s* ppmove, int server) { HUD_PlayerMove(ppmove, server); }

	virtual void PlayerMoveInit(struct playermove_s* ppmove) { HUD_PlayerMoveInit(ppmove); }

	virtual char PlayerMoveTexture(char* name) { return HUD_PlayerMoveTexture(name); }

	virtual void CL_CreateMove(float frametime, struct usercmd_s* cmd, int active) {}

	virtual int CL_IsThirdPerson(void) { return ::CL_IsThirdPerson(); }

	virtual void CL_CameraOffset(float* ofs) { return ::CL_CameraOffset(ofs); }

	virtual void CAM_Think(void) { ::CAM_Think(); }

	virtual void CalcRefdef(ref_params_t* pparams) { ::V_CalcRefdef(pparams); }

	virtual int AddEntity(int type, cl_entity_t* ent, const char* modelname) { return ::HUD_AddEntity(type, ent, modelname); }

	virtual void CreateEntities(void) { ::HUD_CreateEntities(); }

	virtual void DrawNormalTriangles(void) { ::HUD_DrawNormalTriangles(); }

	virtual void DrawTransparentTriangles(void) { ::HUD_DrawTransparentTriangles(); }

	virtual void StudioEvent(const struct mstudioevent_s* event, const cl_entity_t* entity) { ::HUD_StudioEvent(event, entity); }

	virtual void PostRunCmd(struct local_state_s* from, struct local_state_s* to, usercmd_t* cmd, int runfuncs, double time,
	...
	virtual void TxferLocalOverrides(entity_state_t* state, const clientdata_t* client) { ::HUD_TxferLocalOverrides(state, client); }

	virtual void ProcessPlayerState(entity_state_t* dst, const entity_state_t* src) { ::HUD_ProcessPlayerState(dst, src); }

	virtual void TxferPredictionData(entity_state_t* ps, const entity_state_t* pps, clientdata_t* pcd, const clientdata_t* ppcd,
					 weapon_data_t* wd, const weapon_data_t* pwd)
	{
		::HUD_TxferPredictionData(ps, pps, pcd, ppcd, wd, pwd);
	}

	virtual void Demo_ReadBuffer(int size, byte* buffer) { ::Demo_ReadBuffer(size, buffer); }
	...
	virtual int GetHullBounds(int hullnumber, float* mins, float* maxs) { return HUD_GetHullBounds(hullnumber, mins, maxs); }

	virtual void Frame(double time) { HUD_Frame(time); }

	virtual void TempEntUpdate(double frametime, double client_time, double cl_gravity, struct tempent_s** ppTempEntFree,
				   struct tempent_s** ppTempEntActive, int (*Callback_AddVisibleEntity)(cl_entity_t* pEntity),
				   void (*Callback_TempEntPlaySound)(struct tempent_s* pTemp, float damp))
	{
		::HUD_TempEntUpdate(frametime, client_time, cl_gravity, ppTempEntFree, ppTempEntActive, Callback_AddVisibleEntity,
				    Callback_TempEntPlaySound);
	}

	virtual cl_entity_t* GetUserEntity(int index) { return ::HUD_GetUserEntity(index); }

	virtual void VoiceStatus(int entindex, qboolean bTalking) { HUD_VoiceStatus(entindex, bTalking); }

	virtual void DirectorMessage(int iSize, void* pbuf) { HUD_DirectorMessage(iSize, pbuf); }
...
void DLLEXPORT HUD_PlayerMoveInit(struct playermove_s* ppmove) { PM_Init(ppmove); }

char DLLEXPORT HUD_PlayerMoveTexture(char* name) { return PM_FindTextureType(name); }

void DLLEXPORT HUD_PlayerMove(struct playermove_s* ppmove, int server) { PM_Move(ppmove, server); }
...
void DLLEXPORT HUD_DirectorMessage(int iSize, void* pbuf) { gHUD.m_Spectator.DirectorMessage(iSize, pbuf); }
...
bool IsXashFWGS() { return gMobileEngfuncs != NULL; }
