...
int CHudStatusIcons::VidInit(void) { return 1; }
...
void CHudStatusIcons::EnableIcon(const char* pszIconName, unsigned char red, unsigned char green, unsigned char blue)
{
	...
	if (strstr(m_IconList[i].szSpriteName, "grenade"))
	{
		...
		gEngfuncs.pEventAPI->EV_PlaySound(pthisplayer->index, pthisplayer->origin, CHAN_STATIC, "weapons/timer.wav", 1.0, ATTN_NORM, 0,
						  PITCH_NORM);
	}
}
