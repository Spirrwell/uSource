...
void CGameStudioModelRenderer::StudioSetupBones(void)
{
	...
	if (m_fDoInterp && m_pCurrentEntity->latched.sequencetime && (m_pCurrentEntity->latched.sequencetime + 0.2 > m_clTime) &&
...
void GetSequenceInfo(void* pmodel, client_anim_state_t* pev, float* pflFrameRate, float* pflGroundSpeed)
{
	...
	if (pseqdesc->numframes > 1)
	{
		...
		*pflGroundSpeed =
			sqrt(pseqdesc->linearmovement[0] * pseqdesc->linearmovement[0] + pseqdesc->linearmovement[1] * pseqdesc->linearmovement[1] +
...
====================
*/
int R_StudioDrawPlayer(int flags, entity_state_t* pplayer) { return g_StudioRenderer.StudioDrawPlayer(flags, pplayer); }
...
====================
*/
int R_StudioDrawModel(int flags) { return g_StudioRenderer.StudioDrawModel(flags); }
...
====================
*/
void R_StudioInit(void) { g_StudioRenderer.Init(); }
