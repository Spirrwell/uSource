...
void V_CalcNormalRefdef(struct ref_params_s* pparams)
{
	...
	if (pparams->maxclients <= 1)
	{
		...
		{
			pparams->vieworg[i] +=
				scr_ofsx->value * pparams->forward[i] + scr_ofsy->value * pparams->right[i] + scr_ofsz->value * pparams->up[i];
		}
	}
...
void V_ResetChaseCam() { v_resetCamera = true; }
...
{
	static const char* modelmap[][2] = {{"models/p_crossbow.mdl", "models/v_crossbow.mdl"},
...
=============
*/
void V_PunchAxis(int axis, float punch) { g_ev_punchangle[axis] = punch; }
