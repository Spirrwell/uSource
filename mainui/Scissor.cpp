...
void UI::Scissor::PushScissor(int x, int y, int w, int h)
{
	...
	if (scissor.iDepth > 0)
	{
		...
		CropByPreviousScissors(scissor.coordStack[scissor.iDepth - 1], scissor.sizeStack[scissor.iDepth - 1], x, y, w, h);
	}
...
void UI::Scissor::PopScissor()
{
	...
	if (scissor.iDepth > 0)
	{
		EngFuncs::PIC_EnableScissor(scissor.coordStack[scissor.iDepth - 1].x, scissor.coordStack[scissor.iDepth - 1].y,
					    scissor.sizeStack[scissor.iDepth - 1].w, scissor.sizeStack[scissor.iDepth - 1].h);
	}
}
