...
inline bool UI_CursorInRect(Point pos, Size size) { return UI_CursorInRect(pos.x, pos.y, size.w, size.h); }
...
inline void UI_FillRect(Point pos, Size size, const unsigned int color) { UI_FillRect(pos.x, pos.y, size.w, size.h, color); }

void	    UI_DrawRectangleExt(int in_x, int in_y, int in_w, int in_h, const unsigned int color, int outlineWidth,
...
inline void UI_CursorToUIPos(int& x, int& y)
{
	x *= (ScreenWidth / 480);
	y *= (ScreenHeight / 640);
}
...
#define ADD_MENU(cmd, precachefunc, showfunc) static CMenuEntry cmd(#cmd, precachefunc, showfunc)
...
extern CMenu	  gMenu;
extern KeyValues* g_pClientScheme;
