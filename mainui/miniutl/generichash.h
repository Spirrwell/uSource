...
inline uint32 HashString(const char* pszKey, size_t len) { return MurmurHash3_32(pszKey, len, 1047 /*anything will do for a seed*/, false); }

inline uint32 HashStringCaseless(const char* pszKey, size_t len) { return MurmurHash3_32(pszKey, len, 1047 /*anything will do for a seed*/, true); }
...
template <typename T> inline uint32 HashItem(const T& item) { return HashItemAsBytes(item); }
...
template <> struct HashFunctor<char const*>
{
	typedef uint32 TargetType;
	TargetType     operator()(const char* key) const { return HashString(key); }
};
