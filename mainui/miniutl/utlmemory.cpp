...
CUtlMemoryBase::CUtlMemoryBase(int nSizeOfType, int nGrowSize, int nInitAllocationCount)
	: m_pMemory(0), m_nAllocationCount(nInitAllocationCount), m_nGrowSize(nGrowSize), m_unSizeOfElements(nSizeOfType)
{
...
CUtlMemoryBase::CUtlMemoryBase(int nSizeOfType, void* pMemory, int numElements)
	: m_pMemory(pMemory), m_nAllocationCount(numElements), m_unSizeOfElements(nSizeOfType)
{
	Assert(m_unSizeOfElements > 0);
...
CUtlMemoryBase::CUtlMemoryBase(int nSizeOfType, const void* pMemory, int numElements)
	: m_pMemory((void*)pMemory), m_nAllocationCount(numElements), m_unSizeOfElements(nSizeOfType)
{
	Assert(m_unSizeOfElements > 0);
	// Special marker indicating externally supplied modifyable memory
...
CUtlMemoryBase::~CUtlMemoryBase() { Purge(); }
...
//-----------------------------------------------------------------------------
bool CUtlMemoryBase::IsExternallyAllocated() const { return (m_nGrowSize < 0); }

//-----------------------------------------------------------------------------
// is the memory read only?
//-----------------------------------------------------------------------------
bool CUtlMemoryBase::IsReadOnly() const { return (m_nGrowSize == EXTERNAL_CONST_BUFFER_MARKER); }
...
// Size
//-----------------------------------------------------------------------------
int CUtlMemoryBase::NumAllocated() const { return m_nAllocationCount; }

int CUtlMemoryBase::Count() const { return m_nAllocationCount; }

//-----------------------------------------------------------------------------
// Is element index valid?
//-----------------------------------------------------------------------------
bool CUtlMemoryBase::IsIdxValid(int i) const { return (i >= 0) && (i < m_nAllocationCount); }
