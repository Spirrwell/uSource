...
public:
	...
	int SortedFindLessOrEqual(const T& search, bool(__cdecl* pfnLessFunc)(const T& src1, const T& src2, void* pCtx), void* pLessContext,
	...
	// WARNING: The less func for these Sort functions expects -1, 0 for equal, or 1. If you pass only true/false back, you won't get correct
	// sorting.
...
template <class T, size_t MAX_SIZE> class CUtlVectorFixed : public CUtlVector<T, CUtlMemoryFixed<T, MAX_SIZE>>
{
...
//-----------------------------------------------------------------------------
template <class T, size_t MAX_SIZE> class CCopyableUtlVectorFixed : public CUtlVectorFixed<T, MAX_SIZE>
{
...
// Only use this when nesting a CUtlVector() inside of another one of our container classes (i.e a CUtlMap)
//-----------------------------------------------------------------------------
template <class T, class A = CUtlMemory<T>> class CCopyableUtlVector : public CUtlVector<T, A>
{
...
// constructor, destructor
//-----------------------------------------------------------------------------
template <typename T, class A> inline CUtlVector<T, A>::CUtlVector(int growSize, int initSize) : m_Memory(growSize, initSize), m_Size(0) {}
...
template <typename T, class A> inline CUtlVector<T, A>::~CUtlVector() { Purge(); }
...
// Count
//-----------------------------------------------------------------------------
template <typename T, class A> inline int CUtlVector<T, A>::Count() const { return m_Size; }

//-----------------------------------------------------------------------------
// Is element index valid?
//-----------------------------------------------------------------------------
template <typename T, class A> inline bool CUtlVector<T, A>::IsValidIndex(int i) const { return (i >= 0) && (i < m_Size); }

//-----------------------------------------------------------------------------
// Returns in invalid index
//-----------------------------------------------------------------------------
template <typename T, class A> inline int CUtlVector<T, A>::InvalidIndex() { return -1; }

//-----------------------------------------------------------------------------
// Grows the vector
//-----------------------------------------------------------------------------
template <typename T, class A> inline void CUtlVector<T, A>::GrowVector(int num)
{
...
//-----------------------------------------------------------------------------
template <typename T, class A> inline void CUtlVector<T, A>::Reverse()
{
	T*  pBase  = Base();
	int iRight = m_Size - 1;
...
//-----------------------------------------------------------------------------
template <typename T, class A>
inline int CUtlVector<T, A>::SortedFind(const T& search, bool(__cdecl* pfnLessFunc)(const T& src1, const T& src2, void* pCtx),
...
//-----------------------------------------------------------------------------
template <typename T, class A>
inline int CUtlVector<T, A>::SortedFindFirst(const T& search, bool(__cdecl* pfnLessFunc)(const T& src1, const T& src2, void* pCtx),
					     void*    pLessContext) const
{
	int start = 0, stop = Count() - 1;
	while (start <= stop)
...
//-----------------------------------------------------------------------------
template <typename T, class A>
inline int CUtlVector<T, A>::SortedFindLessOrEqual(const T& search, bool(__cdecl* pfnLessFunc)(const T& src1, const T& src2, void* pCtx),
						   void* pLessContext, int start, int stop) const
{
	while (start <= stop)
	{
		int mid = (start + stop) >> 1;
...
//-----------------------------------------------------------------------------
template <typename T, class A>
inline int CUtlVector<T, A>::SortedFindLessOrEqual(const T& search, bool(__cdecl* pfnLessFunc)(const T& src1, const T& src2), int start,
						   int stop) const
{
	while (start <= stop)
	{
...
template <typename T, class A> inline int CUtlVector<T, A>::SortedInsert(const T& src, bool(__cdecl* pfnLessFunc)(const T& src1, const T& src2))
{
...
// Sorts the vector
//-----------------------------------------------------------------------------
template <typename T, class A> inline void CUtlVector<T, A>::Sort(int(__cdecl* pfnCompare)(const T*, const T*))
{
...
// Makes sure we have enough memory allocated to store a requested # of elements
//-----------------------------------------------------------------------------
template <typename T, class A> inline void CUtlVector<T, A>::EnsureCapacity(int num) { m_Memory.EnsureCapacity(num); }

//-----------------------------------------------------------------------------
// Makes sure we have at least this many elements
//-----------------------------------------------------------------------------
template <typename T, class A> inline void CUtlVector<T, A>::EnsureCount(int num)
{
	if (Count() < num)
...
// Shifts elements
//-----------------------------------------------------------------------------
template <typename T, class A> inline void CUtlVector<T, A>::ShiftElementsRight(int elem, int num)
{
...
template <typename T, class A> inline void CUtlVector<T, A>::ShiftElementsLeft(int elem, int num)
{
	DbgAssert(IsValidIndex(elem) || (m_Size == 0) || (num == 0));
	int numToMove = m_Size - elem - num;
	if ((numToMove > 0) && (num > 0))
...
// Adds an element, uses default constructor
//-----------------------------------------------------------------------------
template <typename T, class A> inline int CUtlVector<T, A>::AddToHead() { return InsertBefore(0); }

template <typename T, class A> inline int CUtlVector<T, A>::AddToTail() { return InsertBefore(m_Size); }

template <typename T, class A> inline T* CUtlVector<T, A>::AddToTailGetPtr() { return &Element(AddToTail()); }

template <typename T, class A> inline int CUtlVector<T, A>::InsertAfter(int elem) { return InsertBefore(elem + 1); }

template <typename T, class A> inline int CUtlVector<T, A>::InsertBefore(int elem)
{
...
// Adds an element, uses copy constructor
//-----------------------------------------------------------------------------
template <typename T, class A> inline int CUtlVector<T, A>::AddToHead(const T& src) { return InsertBefore(0, src); }

template <typename T, class A> inline int CUtlVector<T, A>::AddToTail(const T& src) { return InsertBefore(m_Size, src); }

template <typename T, class A> inline int CUtlVector<T, A>::InsertAfter(int elem, const T& src) { return InsertBefore(elem + 1, src); }

template <typename T, class A> inline int CUtlVector<T, A>::InsertBefore(int elem, const T& src)
{
...
// Adds multiple elements, uses default constructor
//-----------------------------------------------------------------------------
template <typename T, class A> inline int CUtlVector<T, A>::AddMultipleToHead(int num) { return InsertMultipleBefore(0, num); }
...
template <typename T, class A> inline int CUtlVector<T, A>::InsertMultipleAfter(int elem, int num, const T* pToCopy)
{
	return InsertMultipleBefore(elem + 1, num, pToCopy);
}

template <typename T, class A> inline void CUtlVector<T, A>::SetCount(int count)
{
	if (count > m_Size)
	{
		int i = m_Size;
...
template <typename T, class A> inline void CUtlVector<T, A>::CopyArray(const T* pArray, int size)
{
	// Can't insert something that's in the list... reallocation may hose us
...
template <typename T, class A> inline void CUtlVector<T, A>::Swap(CUtlVector<T, A>& vec)
{
	m_Memory.Swap(vec.m_Memory);
	SWAP(m_Size, vec.m_Size);
}

template <typename T, class A> inline int CUtlVector<T, A>::AddVectorToTail(CUtlVector const& src)
{
...
// Finds an element (element needs operator== defined)
//-----------------------------------------------------------------------------
template <typename T, class A> inline int CUtlVector<T, A>::Find(const T& src) const
{
...
template <typename T, class A> inline bool CUtlVector<T, A>::HasElement(const T& src) const { return (Find(src) != InvalidIndex()); }

//-----------------------------------------------------------------------------
// Element removal
//-----------------------------------------------------------------------------
template <typename T, class A> inline void CUtlVector<T, A>::FastRemove(int elem)
{
...
template <typename T, class A> inline void CUtlVector<T, A>::Remove(int elem)
{
...
template <typename T, class A> inline bool CUtlVector<T, A>::FindAndRemove(const T& src)
{
	int elem = Find(src);
...
template <typename T, class A> inline bool CUtlVector<T, A>::FindAndFastRemove(const T& src)
{
	int elem = Find(src);
	if (elem != InvalidIndex())
	{
...
template <typename T, class A> inline void CUtlVector<T, A>::RemoveMultipleFromTail(int num)
{
...
template <typename T, class A> inline void CUtlVector<T, A>::RemoveAll()
{
...
template <typename T, class A> inline void CUtlVector<T, A>::Compact() { m_Memory.Purge(m_Size); }
...
public:
	...
	static int __cdecl SortFunc(char* const* sz1, char* const* sz2) { return strcmp(*sz1, *sz2); }
};
...
//-----------------------------------------------------------------------------
#ifdef DBGFLAG_VALIDATE
template <typename T, class A> inline void CUtlVector<T, A>::Validate(CValidator& validator, const char* pchName)
{
#ifdef _WIN32
...
template <typename T, class A> inline void CUtlVector<T, A>::ValidateSelfAndElements(CValidator& validator, const char* pchName)
{
#ifdef _WIN32
	validator.Push(typeid(*this).raw_name(), this, pchName);
#else
