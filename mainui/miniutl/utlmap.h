...
template <typename K, typename T, typename I = int, typename L = bool (*)(const K&, const K&)> class CUtlMap
{
public:
	typedef K KeyType_t;
	...
	IndexType_t GetRoot() const { return m_Tree.Root(); }
	...
	// swap in place
	void Swap(CUtlMap<K, T, I, L>& that) { m_Tree.Swap(that.m_Tree); }
...
protected:
	...
	public:
		...
		bool operator()(const Node_t& left, const Node_t& right) const { return m_LessFunc(left.key, right.key); }
...
//-----------------------------------------------------------------------------
#ifdef DBGFLAG_VALIDATE
template <typename K, typename T, typename I, typename L> void CUtlMap<K, T, I, L>::Validate(CValidator& validator, const char* pchName)
{
#ifdef _WIN32
