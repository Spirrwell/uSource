...
#define UTLMEMORY_TRACK_ALLOC()                                                                                                                      \
	MemAlloc_RegisterAllocation("Sum of all UtlMemory", 0, m_nAllocationCount * sizeof(T), m_nAllocationCount * sizeof(T), 0)
...
template <class T> class CUtlMemory : public CUtlMemoryBase
{
public:
	// constructor, destructor
...
template <class T> inline CUtlMemory<T>::CUtlMemory(T* pMemory, int numElements) : CUtlMemoryBase(sizeof(T), (void*)pMemory, numElements) {}

template <class T> inline CUtlMemory<T>::CUtlMemory(const T* pMemory, int numElements) : CUtlMemoryBase(sizeof(T), pMemory, numElements) {}
...
template <class T> inline const T& CUtlMemory<T>::operator[](int i) const
{
	DbgAssert(IsIdxValid(i));
	return ((T*)m_pMemory)[i];
}
...
template <class T> inline const T& CUtlMemory<T>::Element(int i) const
{
	DbgAssert(IsIdxValid(i));
	return ((T*)m_pMemory)[i];
}
...
// Gets the base address (can change when adding elements!)
//-----------------------------------------------------------------------------
template <class T> inline T* CUtlMemory<T>::Base() { return (T*)m_pMemory; }

template <class T> inline const T* CUtlMemory<T>::Base() const { return (const T*)m_pMemory; }
