...
CEventCallback::CEventCallback() : pExtra(0), type(CB_OLD_EXTRA), szName(0) { memset(&u, 0, sizeof(u)); }

CEventCallback::CEventCallback(EventCallback cb, void* ex) : pExtra(ex), type(CB_OLD_EXTRA), szName(0)
{
	memset(&u, 0, sizeof(u));
	u.cb = cb;
}
...
CEventCallback::CEventCallback(VoidCallback cb) : pExtra(0), type(CB_OLD_VOID), szName(0)
{
	memset(&u, 0, sizeof(u));
	u.voidCb = cb;
}

CEventCallback::CEventCallback(IHCallback cb, void* ex) : pExtra(ex), type(CB_IH_EXTRA), szName(0)
{
	memset(&u, 0, sizeof(u));
	u.itemsHolderCb = cb;
}

CEventCallback::CEventCallback(VoidIHCallback cb) : pExtra(0), type(CB_IH_VOID), szName(0)
{
	memset(&u, 0, sizeof(u));
