...
CMenuTable::CMenuTable()
	: BaseClass(), bFramedHintText(false), bAllowSorting(false), bShowScrollBar(true), szHeaderTexts(), szBackground(), szUpArrow(UI_UPARROW),
	  szUpArrowFocus(UI_UPARROWFOCUS), szUpArrowPressed(UI_UPARROWPRESSED), szDownArrow(UI_DOWNARROW), szDownArrowFocus(UI_DOWNARROWFOCUS),
	  szDownArrowPressed(UI_DOWNARROWPRESSED), iTopItem(0), iScrollBarSliding(false), iHighlight(-1), iCurItem(0), m_iLastItemMouseChange(0),
	  m_iSortingColumn(-1), m_pModel(NULL)
{
...
bool CMenuTable::KeyDown(int key)
{
	...
	case K_MOUSE1:
	{
		...
		{
			// ADAMIX
			if (UI_CursorInRect(upArrow.x, upArrow.y + arrow.h, arrow.w, sbarPos.y - upArrow.y - arrow.h) ||
			    UI_CursorInRect(upArrow.x, sbarPos.y + sbarSize.h, arrow.w, downArrow.y - sbarPos.y - sbarSize.h))
			{
...
void CMenuTable::Draw()
{
	...
	if (iScrollBarSliding)
	{
		...
		if ((((dist / 2) > (m_scChSize / 2)) || ((dist / 2) < (m_scChSize / 2))) && iTopItem <= (m_pModel->GetRows() - iNumRows) &&
		    iTopItem >= 0)
		{
			//_Event( QM_CHANGED );
