	...
	else
	{
		...
		case K_MOUSE1:
		{
			...
			if (UI_CursorInRect(m_scPos.x, y, m_scSize.w, m_scSize.h))
			{
				...
				charpos = g_FontMgr->CutText(font, szBuffer + newScroll, m_scChSize, uiStatic.cursorX - x, false, false, &w,
							     &remaining);
...
void CMenuField::Draw(void)
{
	...
	if ((uiStatic.realTime & 499) < 250)
		UI_DrawString(font, x + cursorOffset, y, cursor_char_width, m_scSize.h, cursor_char, colorBase, m_scChSize, QM_LEFT,
	...
	case QM_HIGHLIGHTIFFOCUS:
		...
		if ((uiStatic.realTime & 499) < 250)
			UI_DrawString(font, x + cursorOffset, y, cursor_char_width, m_scSize.h, cursor_char, colorFocus, m_scChSize, QM_LEFT,
				      textflags | ETF_FORCECOL);
		break;
	case QM_PULSEIFFOCUS:
	{
		...
		if ((uiStatic.realTime & 499) < 250)
			UI_DrawString(font, x + cursorOffset, y, cursor_char_width, m_scSize.h, cursor_char, color, m_scChSize, QM_LEFT,
				      textflags | ETF_FORCECOL);
