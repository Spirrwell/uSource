...
Frame::Frame(const char* name)
	: CMenuBaseWindow(name), m_titleText("Frame"), m_titleColor(PackRGBA(255, 255, 255, 255)), m_titleFont(FindFontByName("DefaultVerySmall")),
	  m_dockMode(EDockMode::NONE), m_margin(5), resizing(false), m_roundedCorners(true), m_dragging(false)
{
...
void Frame::Draw()
{
	...
	if (m_roundedCorners)
	{
		...
		EngFuncs::FillRGBA(this->m_scPos.x + cornerRadius, this->m_scPos.y, this->m_scSize.w - (cornerRadius * 2), cornerRadius,
				   m_backgroundColor.r, m_backgroundColor.g, m_backgroundColor.b, m_backgroundColor.a);
		EngFuncs::FillRGBA(this->m_scPos.x + cornerRadius, this->m_scPos.y + (m_scSize.h - cornerRadius),
				   this->m_scSize.w - (cornerRadius * 2), cornerRadius, m_backgroundColor.r, m_backgroundColor.g, m_backgroundColor.b,
				   m_backgroundColor.a);
		...
		UI_DrawPic(this->m_scPos + Point(m_scSize.w - cornerRadius, 0), cornerSize, m_backgroundColor.rgba, "resource/icon_corner0",
			   QM_DRAWTRANS);
		UI_DrawPic(this->m_scPos + Point(0, m_scSize.h - cornerRadius), cornerSize, m_backgroundColor.rgba, "resource/icon_corner180",
			   QM_DRAWTRANS);
		UI_DrawPic(this->m_scPos + Point(m_scSize.w - cornerRadius, m_scSize.h - cornerRadius), cornerSize, m_backgroundColor.rgba,
			   "resource/icon_corner270", QM_DRAWTRANS);
	}
	else
	{
