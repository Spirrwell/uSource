...
CMenuSpinControl::CMenuSpinControl()
	: BaseClass(), m_szBackground(), m_szLeftArrow(), m_szRightArrow(), m_szLeftArrowFocus(), m_szRightArrowFocus(), m_flMinValue(0),
	  m_flMaxValue(1), m_flCurValue(0), m_flRange(0.1), m_pModel(NULL), m_iFloatPrecision(0), m_szDisplay()
{
	m_szBackground	    = NULL;
...
void CMenuSpinControl::Draw(void)
{
	...
	UI_DrawString(font, m_scPos.x - UI_OUTLINE_WIDTH, textHeight, m_scSize.w + UI_OUTLINE_WIDTH * 2, m_scChSize, szName, uiColorHelp, m_scChSize,
		      QM_LEFT, textflags | ETF_FORCECOL);
...
void CMenuSpinControl::SetDisplayPrecision(short precision) { m_iFloatPrecision = precision; }
...
void CMenuSpinControl::ForceDisplayString(const char* display) { Q_strncpy(m_szDisplay, display, CS_SIZE); }
