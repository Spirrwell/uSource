...
CMenuItemsHolder::CMenuItemsHolder()
	: BaseClass(), m_iCursor(0), m_iCursorPrev(0), m_pItems(), m_events(), m_bInit(false), m_bWrapCursor(true), m_szResFile(0)
{
	;
}
...
bool CMenuItemsHolder::KeyDown(int key) { return CMenuItemsHolder::Key(key, true); }

bool CMenuItemsHolder::KeyUp(int key) { return CMenuItemsHolder::Key(key, false); }
...
{
	FOR_EACH_VEC(m_pItems, i) { m_pItems[i]->Think(); }
}
...
bool CMenuItemsHolder::KeyValueData(const char* key, const char* data) { return BaseClass::KeyValueData(key, data); }

Point CMenuItemsHolder::GetPositionOffset() const { return m_scPos; }
