...
CStbFont::CStbFont() : CBaseFont(), m_ABCCache(0, 0), m_szRealFontFile(), m_pFontData(NULL) { SetDefLessFunc(m_ABCCache); }

CStbFont::~CStbFont() { delete[] m_pFontData; }
...
bool CStbFont::Create(const char* name, int tall, int weight, int blur, float brighten, int outlineSize, int scanlineOffset, float scanlineScale,
		      int flags)
{
...
bool CStbFont::HasChar(int ch) const { return true; }
