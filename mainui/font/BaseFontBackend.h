...
public:
	...
	virtual bool Create(const char* name, int tall, int weight, int blur, float brighten, int outlineSize, int scanlineOffset,
			    float scanlineScale, int flags)				= 0;
...
private:
	...
	struct glyph_t
	{
		...
		bool operator<(const glyph_t& a) const { return ch < a.ch; }
	};
