...
CWinAPIFont::CWinAPIFont() : CBaseFont(), m_ABCCache(0, 0) { SetDefLessFunc(m_ABCCache); }
...
bool CWinAPIFont::Create(const char* name, int tall, int weight, int blur, float brighten, int outlineSize, int scanlineOffset, float scanlineScale,
			 int flags)
{
	...
	m_hFont = ::CreateFontA(m_iTall, 0, 0, 0, m_iWeight, m_iFlags & FONT_ITALIC, m_iFlags & FONT_UNDERLINE, m_iFlags & FONT_STRIKEOUT, charset,
				OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, PROOF_QUALITY, DEFAULT_PITCH | FF_DONTCARE, font.lfFaceName);
...
bool CWinAPIFont::HasChar(int ch) const { return true; }
