...
public:
	...
	bool IsCellTextWrapped(int line, int column) { return IsLineUsable(line); }

	bool IsLineUsable(int line) { return keysBind[line][0] != 0; }
...
void CMenuKeysModel::OnActivateEntry(int line) { uiControls.EnterGrabMode(); }

void CMenuKeysModel::OnDeleteEntry(int line) { uiControls.UnbindEntry(); }
...
bool CMenuControls::CGrabKeyMessageBox::KeyDown(int key) { return true; }
...
void CMenuControls::_Init(void)
{
	...
	AddButton(L("GameUI_OK"), L("Save changed and return to configuration menu"), PC_DONE, VoidCb(&CMenuControls::SaveAndPopMenu));
	AddButton(L("GameUI_Cancel"), L("Discard changes and return to configuration menu"), PC_CANCEL, VoidCb(&CMenuControls::Cancel));
	AddItem(keysList);
}
...
=================
*/
void UI_Controls_Precache(void) { EngFuncs::PIC_Load(ART_BANNER); }
...
=================
*/
void UI_Controls_Menu(void) { uiControls.Show(); }
