...
public:
	...
	const char* GetCellText(int line, int column) override { return modsDescription[line][column]; }
...
void CMenuCustomGame::_Init(void)
{
	...
	AddItem(background);
	AddItem(banner);
	load = AddButton(L("Activate"), L("Activate selected custom game"), PC_ACTIVATE, msgBox.MakeOpenEvent());

	go2url = AddButton(L("Visit web site"), L("Visit the web site of game developers"), PC_VISIT_WEB_SITE, MenuCb(&CMenuCustomGame::Go2Site));
...
=================
*/
void UI_CustomGame_Precache(void) { EngFuncs::PIC_Load(ART_BANNER); }
