...
public:
	void Update() override
	{
		...
		for (unsigned int i = 0; EngFuncs::GetRenderers(i, temp.shortName, sizeof(temp.shortName), temp.readable, sizeof(temp.readable)); i++)
		{
...
void CMenuVidModes::Draw()
{
	if (testModeMsgBox.IsVisible())
	{
		...
		{
			snprintf(testModeMsg, sizeof(testModeMsg) - 1, L("Keep this resolution? %i seconds remaining"),
...
=================
*/
void UI_VidModes_Precache(void) { EngFuncs::PIC_Load(ART_BANNER); }
...
=================
*/
void UI_VidModes_Menu(void) { uiVidModes.Show(); }
