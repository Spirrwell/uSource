...
static const char* uiCreditsDefault[] = {"", "Copyright XashXT Group 2017 (C)", "Copyright Flying With Gauss 2017 (C)", 0};
...
CMenuCredits::~CMenuCredits() { delete buffer; }
...
void CMenuCredits::Draw(void)
{
	...
	for (i = 0; i < uiCredits.numLines && uiCredits.credits[i]; i++, y += h)
	{
		...
		{
			if (!uiCredits.fadeTime)
				uiCredits.fadeTime = (gpGlobals->time * 1000);
			color = UI_FadeAlpha(uiCredits.fadeTime, uiCredits.showTime);
			if (UnpackAlpha(color))
				UI_DrawString(uiStatic.hDefaultFont, 0, (ScreenHeight - h) / 2, ScreenWidth, h, uiCredits.credits[i], color, h,
					      QM_CENTER, ETF_SHADOW | ETF_FORCECOL);
		}
		else
			UI_DrawString(uiStatic.hDefaultFont, 0, y, ScreenWidth, h, uiCredits.credits[i], uiColorWhite, h, QM_CENTER, ETF_SHADOW);
	}
...
bool CMenuCredits::KeyUp(int key) { return true; }
...
int UI_CreditsActive(void) { return uiCredits.active && uiCredits.finalCredits; }
...
=================
*/
void UI_Credits_Menu(void) { uiCredits.Show(); }
