...
{
public:
	CMenuSavePreview() : CMenuBaseItem() { iFlags = QMF_INACTIVE; }
...
public:
	...
	const char*  GetCellText(int line, int column) override { return m_szCells[line][column]; }
...
void CMenuSavesListModel::Update(void)
{
	...
	if (saveName[0][0] == 0 || !CL_IsActive())
		...
		uiLoadGame.save.SetGrayed(false);

	if (delName[0][0] == 0)
		uiLoadGame.remove.SetGrayed(true);
	else
		uiLoadGame.remove.SetGrayed(false);
}

void CMenuSavesListModel::OnDeleteEntry(int line) { uiLoadGame.msgBox.Show(); }
...
=================
*/
void UI_LoadGame_Menu(void) { UI_LoadSaveGame_Menu(false); }

void UI_SaveGame_Menu(void) { UI_LoadSaveGame_Menu(true); }
