...
void CMenuTouchButtons::CMenuButtonPreview::Draw()
{
	...
	EngFuncs::PIC_Set(textureId, (int)parent->red.GetCurrentValue(), (int)parent->green.GetCurrentValue(), (int)parent->blue.GetCurrentValue(),
...
void CMenuTouchButtons::SaveButton()
{
	...
	else
	{
		...
		EngFuncs::ClientCmd(0, command);
		snprintf(command, sizeof(command), "touch_setcolor \"%s\" %u %u %u %u\n", selectedName, (uint)red.GetCurrentValue(),
			 (uint)green.GetCurrentValue(), (uint)blue.GetCurrentValue(), (uint)alpha.GetCurrentValue());
		EngFuncs::ClientCmd(1, command);
	}
...
void UI_TouchButtons_GetButtonList() { uiTouchButtons.model.Update(); }
