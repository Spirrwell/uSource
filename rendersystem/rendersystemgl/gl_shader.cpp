...
CGenericShader_GL::CGenericShader_GL(GLenum type)
	: m_shaderIndex(0), m_init(false), m_compiled(false), m_type(type), m_lineLengths(nullptr), m_numLines(0), m_sourceCode(nullptr)
{
...
IShaderProgram* CRenderSystem_GL::CreateShaderProgram(const char* name) { return new CShaderProgram_GL(name); }

CShaderProgram_GL::CShaderProgram_GL(const char* name)
	: m_fragShader(nullptr), m_vertShader(nullptr), m_geomShader(nullptr), m_programIndex(0), m_linked(false), m_stencil(false),
	  m_currentColorBuffer(0), m_depth(0), m_maxRenderTargets(0)
{
	m_name	       = strdup(name);
...
bool CShaderProgram_GL::CheckParamType(GLint param, GLenum type) { return false; }

bool CShaderProgram_GL::CheckUniformType(GLint param, GLenum type) { return false; }

const char* CShaderProgram_GL::GetName() const { return m_name; }

void CShaderProgram_GL::BindVertexShader(IVertexShader* pShader) { glAttachShader(m_programIndex, ((CVertexShader_GL*)pShader)->m_shaderIndex); }

void CShaderProgram_GL::BindFragShader(IFragShader* pShader) { glAttachShader(m_programIndex, ((CFragShader_GL*)pShader)->m_shaderIndex); }

void CShaderProgram_GL::BindGeometryShader(IGeomShader* pShader) { glAttachShader(m_programIndex, ((CGeomShader_GL*)pShader)->m_shaderIndex); }
...
void CShaderProgram_GL::AddRenderTarget(const char* param, int& index, ITexture* pTexture)
{
	...
	static GLenum colorbuffer_map[] = {GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT1, GL_COLOR_ATTACHMENT2, GL_COLOR_ATTACHMENT3,
...
void CShaderProgram_GL::PreDraw()
{
	...
	{
		AssertMsg(x.index < 10, "Index of output buffer was greater than 10! Max of 8 color attachments, 1 depth attachment and 1 stencil "
