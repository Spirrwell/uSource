...
CTexture_GL* CTexture_GL::Create(int w, int h, ETextureFlags flags, ETextureFormat fmt) { return nullptr; }
...
CTexture_GL::CTexture_GL(int w, int h, ETextureFlags flags, ETextureFormat fmt)
	: m_flags(TEXFLAGS_NONE), m_renderBuffer(0), m_format(fmt), m_width(w), m_height(h), m_data(nullptr), m_texture(0)
{
	...
	static GLenum texture_format_map[] = {
		GL_RGB, GL_RGB, GL_RGBA, GL_RGBA, GL_RED, GL_RED, GL_RGBA, GL_RGBA,
	};
...
ETextureFlags CTexture_GL::Flags() const { return TEXFLAGS_STENCIL; }

ETextureFormat CTexture_GL::GetFormat() const { return ETextureFormat::RGB888; }

ITexture* CTexture_GL::Convert(ETextureFormat fmt) { return nullptr; }
...
void CTexture_GL::LoadFromCompressedImage(int w, int h, ETextureFormat fmt, void* buffer) { Assert(buffer); }
