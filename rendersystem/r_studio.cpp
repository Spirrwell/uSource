...
static vec3_t hullcolor[8] = {
	{1.0f, 1.0f, 1.0f}, {1.0f, 0.5f, 0.5f}, {0.5f, 1.0f, 0.5f}, {1.0f, 1.0f, 0.5f},
	{0.5f, 0.5f, 1.0f}, {1.0f, 0.5f, 1.0f}, {0.5f, 1.0f, 1.0f}, {1.0f, 1.0f, 1.0f},
};
...
===============
*/
static cl_entity_t* pfnGetCurrentEntity(void) { return RI.currententity; }
...
===============
*/
static model_t* pfnMod_ForName(const char* model, int crash) { return gEngfuncs.Mod_ForName(model, crash, false); }
...
===============
*/
static cl_entity_t* pfnGetViewEntity(void) { return gEngfuncs.GetViewModel(); }
...
===============
*/
static model_t* R_GetChromeSprite(void) { return gEngfuncs.GetDefaultSprite(REF_CHROME_SPRITE); }
...
===============
*/
static float**** pfnStudioGetBoneTransform(void) { return (float****)g_studio.bonestransform; }
...
===============
*/
static float**** pfnStudioGetLightTransform(void) { return (float****)g_studio.lighttransform; }
...
===============
*/
static float*** pfnStudioGetAliasTransform(void) { return NULL; }
...
===============
*/
static float*** pfnStudioGetRotationMatrix(void) { return (float***)g_studio.rotationmatrix; }
...
static void R_StudioSetColorBegin(short* ptricmds, vec3_t* pstudionorms)
{
	...
	else
	{
		if (RI.currententity->curstate.rendermode == kRenderTransColor)
		{
			...
			pglColor4f(lv[0], lv[1], lv[2], tr.blend);
	}
}
...
===============
*/
void R_StudioSetCullState(int iCull) { g_iBackFaceCull = iCull; }
...
_inline void R_StudioDrawArrays(uint startverts, uint startelems)
{
	...
	if (pglDrawRangeElements)
		pglDrawRangeElements(GL_TRIANGLES, startverts, g_studio.numverts, g_studio.numelems - startelems, GL_UNSIGNED_SHORT,
...
void R_StudioResetPlayerModels(void) { memset(g_studio.player_models, 0, sizeof(g_studio.player_models)); }
...
===============
*/
int R_StudioGetForceFaceFlags(void) { return g_nForceFaceFlags; }
...
===============
*/
void R_StudioSetForceFaceFlags(int flags) { g_nForceFaceFlags = flags; }
...
===============
*/
void R_StudioSetRenderModel(model_t* model) { RI.currentmodel = model; }
...
===============
*/
void R_StudioSetChromeOrigin(void) { VectorCopy(RI.vieworg, g_studio.chrome_origin); }
...
static int R_StudioDrawModel(int flags)
{
	...
	if (RI.currententity->curstate.renderfx == kRenderFxDeadPlayer)
	{
		...
		if (RI.currententity->curstate.renderamt <= 0 || RI.currententity->curstate.renderamt > ENGINE_GET_PARM(PARM_MAX_CLIENTS))
			return 0;
...
static model_t* pfnModelHandle(int modelindex) { return gEngfuncs.pfnGetModelByIndex(modelindex); }

static void* pfnMod_CacheCheck(struct cache_user_s* c) { return gEngfuncs.Mod_CacheCheck(c); }

static void* pfnMod_StudioExtradata(model_t* mod) { return gEngfuncs.Mod_Extradata(mod_studio, mod); }

static void pfnMod_LoadCacheFile(const char* path, struct cache_user_s* cu) { gEngfuncs.Mod_LoadCacheFile(path, cu); }

static cvar_t* pfnGetCvarPointer(const char* name) { return (cvar_t*)gEngfuncs.pfnGetCvarPointer(name, 0); }

static void* pfnMod_Calloc(int number, size_t size) { return gEngfuncs.Mod_Calloc(number, size); }
...
public:
	...
	virtual void* Mem_Calloc(int number, size_t size) { return pfnMod_Calloc(number, size); }

	virtual void* Cache_Check(struct cache_user_s* c) { return pfnMod_CacheCheck(c); }

	virtual void LoadCacheFile(const char* path, struct cache_user_s* cu) { pfnMod_LoadCacheFile(path, cu); }

	virtual struct model_s* Mod_ForName(const char* name, int crash_if_missing) { return pfnMod_ForName(name, crash_if_missing); }

	virtual void* Mod_Extradata(struct model_s* mod) { return pfnMod_StudioExtradata(mod); }

	virtual struct model_s* GetModelByIndex(int index) { return pfnModelHandle(index); }

	virtual struct cl_entity_s* GetCurrentEntity(void) { return pfnGetCurrentEntity(); }

	virtual struct player_info_s* PlayerInfo(int index) { return pfnPlayerInfo(index); }

	virtual struct entity_state_s* GetPlayerState(int index) { return R_StudioGetPlayerState(index); }

	virtual struct cl_entity_s* GetViewEntity(void) { return pfnGetViewEntity(); }

	virtual void GetTimes(int* framecount, double* current, double* old) { pfnGetEngineTimes(framecount, current, old); }

	virtual struct cvar_s* GetCvar(const char* name) { return pfnGetCvarPointer(name); }

	virtual void GetViewInfo(float* origin, float* upv, float* rightv, float* vpnv) { pfnGetViewInfo(origin, upv, rightv, vpnv); }

	virtual struct model_s* GetChromeSprite(void) { return R_GetChromeSprite(); }

	virtual void GetModelCounters(int** s, int** a) { pfnGetModelCounters(s, a); }

	virtual void GetAliasScale(float* x, float* y) { pfnGetAliasScale(x, y); }

	virtual float**** StudioGetBoneTransform(void) { return pfnStudioGetBoneTransform(); }

	virtual float**** StudioGetLightTransform(void) { return pfnStudioGetLightTransform(); }

	virtual float*** StudioGetAliasTransform(void) { return pfnStudioGetAliasTransform(); }

	virtual float*** StudioGetRotationMatrix(void) { return pfnStudioGetRotationMatrix(); }
	...
	virtual int StudioCheckBBox(void) { return R_StudioCheckBBox(); }

	virtual void StudioDynamicLight(struct cl_entity_s* ent, struct alight_s* plight) { R_StudioDynamicLight(ent, plight); }

	virtual void StudioEntityLight(struct alight_s* plight) { R_StudioEntityLight(plight); }

	virtual void StudioSetupLighting(struct alight_s* plighting) { R_StudioSetupLighting(plighting); }

	virtual void StudioDrawPoints(void) { R_StudioDrawPoints(); }

	virtual void StudioDrawHulls(void) { R_StudioDrawHulls(); }

	virtual void StudioDrawAbsBBox(void) { R_StudioDrawAbsBBox(); }

	virtual void StudioDrawBones(void) { R_StudioDrawBones(); }

	virtual void StudioSetupSkin(void* ptexturehdr, int index) { R_StudioSetupSkin((studiohdr_t*)ptexturehdr, index); }

	virtual void StudioSetRemapColors(int top, int bottom) { R_StudioSetRemapColors(top, bottom); }

	virtual struct model_s* SetupPlayerModel(int index) { return R_StudioSetupPlayerModel(index); }

	virtual void StudioClientEvents(void) { R_StudioClientEvents(); }

	virtual int GetForceFaceFlags(void) { return R_StudioGetForceFaceFlags(); }

	virtual void SetForceFaceFlags(int flags) { R_StudioSetForceFaceFlags(flags); }

	virtual void StudioSetHeader(void* header) { R_StudioSetHeader((studiohdr_t*)header); }

	virtual void SetRenderModel(struct model_s* model) { R_StudioSetRenderModel(model); }

	virtual void SetupRenderer(int rendermode) { R_StudioSetupRenderer(rendermode); }

	virtual void RestoreRenderer(void) { R_StudioRestoreRenderer(); }

	virtual void SetChromeOrigin(void) { R_StudioSetChromeOrigin(); }

	virtual int IsHardware(void) { return pfnIsHardware(); }

	virtual void GL_StudioDrawShadow(void) { ::GL_StudioDrawShadow(); }

	virtual void GL_SetRenderMode(int mode) { ::GL_SetRenderMode(mode); }

	virtual void StudioSetRenderamt(int iRenderamt) { R_StudioSetRenderamt(iRenderamt); }

	virtual void StudioSetCullState(int iCull) { R_StudioSetCullState(iCull); }

	virtual void StudioRenderShadow(int iSprite, float* p1, float* p2, float* p3, float* p4) { R_StudioRenderShadow(iSprite, p1, p2, p3, p4); }
};
