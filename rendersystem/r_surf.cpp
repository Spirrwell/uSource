...
static void SubdividePolygon_r(msurface_t* warpface, int numverts, float* verts)
{
	...
	poly		= static_cast<glpoly_t*>(Mem_Calloc(loadmodel->mempool, sizeof(glpoly_t) + (numverts - 4) * VERTEXSIZE * sizeof(float)));
...
void GL_BuildPolygonFromSurface(model_t* mod, msurface_t* fa)
{
	...
	poly	       = static_cast<glpoly_t*>(Mem_Realloc(mod->mempool, poly, sizeof(glpoly_t) + (lnumverts - 4) * VERTEXSIZE * sizeof(float)));
	poly->next     = fa->polys;
	poly->flags    = fa->flags;
	fa->polys      = poly;
...
void R_BlendLightmaps(void)
{
	...
	if (CVAR_TO_BOOL(r_dynamic))
	{
		...
		for (surf = newsurf; surf != NULL; surf = surf->info->lightmapchain)
		{
			if (surf->polys)
			{
				DrawGLPolyChain(surf->polys, (surf->light_s - surf->info->dlight_s) * (1.0f / (float)BLOCK_SIZE),
...
void R_GenerateVBO()
{
	...
	vbos.textures  = static_cast<vbotexture_t*>(Mem_Calloc(vbos.mempool, numtextures * numlightmaps * sizeof(vbotexture_t)));
	vbos.surfdata  = static_cast<vbosurfdata_t*>(Mem_Calloc(vbos.mempool, WORLDMODEL->numsurfaces * sizeof(vbosurfdata_t)));
		...
		for (j = 0; j < numtextures; j++)
		{
			...
			for (i = 0; i < WORLDMODEL->numsurfaces; i++)
			{
				...
				if (vbo->array_len + surf->polys->numverts > USHRT_MAX)
				{
					...
					vbo->next    = static_cast<vboarray_s*>(Mem_Calloc(vbos.mempool, sizeof(vboarray_t)));
					vbo	     = vbo->next;
					vbotex->next = static_cast<vbotexture_s*>(Mem_Calloc(vbos.mempool, sizeof(vbotexture_t)));
					vbotex	     = vbotex->next;
	...
	for (k = 0; k < numlightmaps; k++)
	{
		...
		for (j = 0; j < numtextures; j++)
		{
			...
			vbotex->indexarray	   = static_cast<unsigned short*>(Mem_Calloc(vbos.mempool, sizeof(unsigned short) * 6 * vbotex->len));
			...
			for (i = 0; i < WORLDMODEL->numsurfaces; i++)
			{
				...
				if (len + surf->polys->numverts > USHRT_MAX)
				{
					...
					vbotex = vbotex->next;
					vbotex->indexarray =
						static_cast<unsigned short*>(Mem_Calloc(vbos.mempool, sizeof(unsigned short) * 6 * vbotex->len));
					vbotex->lightmaptexturenum = k;
	...
	pglBufferDataARB(GL_ARRAY_BUFFER_ARB, sizeof(vbovertex_t) * DECAL_VERTS_CUT * MAX_RENDER_DECALS, vbos.decaldata->decalarray,
			 GL_DYNAMIC_DRAW_ARB);

	// preallocate dlight arrays
	vbos.dlight_index = static_cast<unsigned short*>(Mem_Calloc(vbos.mempool, maxindex * sizeof(unsigned short) * 6));

	// select maximum possible length for dlight
	vbos.dlight_tc = static_cast<vec2_t*>(
		Mem_Calloc(vbos.mempool, sizeof(vec2_t) * (int)(vbos.arraylist->next ? USHRT_MAX + 1 : vbos.arraylist->array_len + 1)));

	if (CVAR_TO_BOOL(r_vbo_dlightmode))
	{
		pglGenBuffersARB(1, &vbos.dlight_vbo);
		pglBindBufferARB(GL_ARRAY_BUFFER_ARB, vbos.dlight_vbo);
		pglBufferDataARB(GL_ARRAY_BUFFER_ARB, sizeof(vec2_t) * (int)(vbos.arraylist->next ? USHRT_MAX + 1 : vbos.arraylist->array_len + 1),
				 vbos.dlight_tc, GL_STREAM_DRAW_ARB);
...
void R_AddDecalVBO(decal_t* pdecal, msurface_t* surf)
{
	...
	pglBindBufferARB(GL_ARRAY_BUFFER_ARB, vbos.decaldata->decalvbo);
	pglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, decalindex * sizeof(vbovertex_t) * DECAL_VERTS_CUT, sizeof(vbovertex_t) * numVerts,
	...
	if (vbotex->dlightchain)
	{
		...
		for (surf = newsurf = vbotex->dlightchain; surf; surf = surf->info->lightmapchain)
		{
			...
			for (index = indexbase + 2; index < indexbase + surf->polys->numverts; index++)
			{
				...
				vbos.dlight_tc[index][0] =
					surf->polys->verts[index - indexbase][5] - (surf->light_s - info->dlight_s) * (1.0f / (float)BLOCK_SIZE);
				vbos.dlight_tc[index][1] =
					surf->polys->verts[index - indexbase][6] - (surf->light_t - info->dlight_t) * (1.0f / (float)BLOCK_SIZE);
			}

			if (vbos.dlight_vbo)
			{
				pglBindBufferARB(GL_ARRAY_BUFFER_ARB, vbos.dlight_vbo);
				pglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, sizeof(vec2_t) * indexbase, sizeof(vec2_t) * surf->polys->numverts,
			...
			for (lightmapchain = vbos.decaldata->lm[k]; lightmapchain; lightmapchain = lightmapchain->info->lightmapchain)
			{
				...
				for (pdecal = lightmapchain->pdecals; pdecal; pdecal = pdecal->pnext)
				{
					...
					if (vbos.decaldata->decals[decalindex].numVerts == -1)
					{
						...
						pglDrawArrays(GL_TRIANGLE_FAN, decalindex * DECAL_VERTS_CUT,
