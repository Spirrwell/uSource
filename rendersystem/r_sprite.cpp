...
static const dframetype_t* R_SpriteLoadFrame(model_t* mod, const void* pin, mspriteframe_t** ppframe, int num)
{
	...
	*ppframe		    = pspriteframe;
	...
	return reinterpret_cast<const dframetype_t*>((const byte*)pin + sizeof(dspriteframe_t) + pinframe.width * pinframe.height * bytes);
}
...
void Mod_LoadMapSprite(model_t* mod, const void* buffer, size_t size, qboolean* loaded)
{
	...
	psprite		= static_cast<msprite_t*>(Mem_Calloc(mod->mempool, sizeof(msprite_t) + (numframes - 1) * sizeof(psprite->frames)));
	...
	for (i = xl = yl = 0; i < numframes; i++)
	{
		...
		psprite->frames[i].frameptr = static_cast<mspriteframe_t*>(Mem_Calloc(mod->mempool, sizeof(mspriteframe_t)));
