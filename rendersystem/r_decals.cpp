	...
	*pcount = 0;
	...
	while (pDecal)
	{
		...
		if (!FBitSet(pDecal->flags, FDECAL_PERMANENT))
		{
			...
			Vector2Set(vDecalMax, DotProduct(testPosition[0], testBasis[0]) - pDecal->dx + 0.5f,
				   DotProduct(testPosition[1], testBasis[1]) - pDecal->dy + 0.5f);
...
glpoly_t* R_DecalCreatePoly(decalinfo_t* decalinfo, decal_t* pdecal, msurface_t* surf)
{
	...
	poly	       = static_cast<glpoly_t*>(Mem_Calloc(r_temppool, sizeof(glpoly_t) + (lnumverts - 4) * VERTEXSIZE * sizeof(float)));
