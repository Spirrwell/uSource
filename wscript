...
def options(opt):
    ...
    grp.add_option('--no-sse', action='store_true', dest='NO_SSE', default=False,
                   help='Explicitly disables the use of SSE, even on x86_64 targets')
    ...
    grp.add_option('--enable-scripting', action='store_true', dest='ENABLE_SCRIPTING',
                   help='Enables scripting and builds the scriptsystem DLL')

    grp.add_option('--enable-luajit', action='store_true', dest='ENABLE_LUA', default=False,
    ...
    grp.add_option('--enable-new-renderer', action='store_true', dest='ENABLE_RENDERER2', default=False,
                   help='Enables the use of the new renderer')
    grp.add_option('--enable-vulkan', action='store_true', dest='ENABLE_VULKAN', default=False,
                   help='Enables the use of the vulkan backend')

    grp.add_option('--dev', action='store_true', dest='DEV', default=False,
                   help='Force development build macros to be enabled. This is enabled implicitly when building in debug mode')

    grp.add_option('--enable-remote-control', action='store_true', dest='ENABLE_RCSYS',
                   help='Enables the experimental remote control system')
...
def configure(conf):
    ...
    compiler_c_cxx_flags = {
        'common': {
            ...
            'clang': ['-g', '-std=c++17', '-gdwarf-2', '-fvisibility=default', '-Wall', '-Wextra',
                      '-Wno-unused-parameter', '-fpermissive', '-Wno-unused-function', '-DENABLE_XPROF'],
            'gcc': ['-g', '-std=c++17', '-Wno-expansion-to-defined', '-fvisibility=default', '-Wno-attributes',
                    '-Wno-write-strings', '-Wall', '-Wextra', '-Wno-unused-parameter', '-fpermissive',
                    '-Wno-unused-function', '-Wno-unused-variable', '-Wno-missing-field-initializers',
                    '-Wno-invalid-offsetof', '-Wno-sign-compare', '-Wno-reorder', '-Wno-unknown-pragmas',
                    '-DENABLE_XPROF']
        },
        'fast': {
            'msvc': ['/O2', '/Oy', '/MT'],  # todo: check /GL /LTCG
            'gcc': ['-Ofast', '-march=native', '-funsafe-math-optimizations', '-funsafe-loop-optimizations',
                    '-fomit-frame-pointer'],
    ...
    conf.env.append_unique('DEFINES',
                           'XASH_BUILD_COMMIT="{0}"'.format(conf.env.GIT_VERSION if conf.env.GIT_VERSION else 'notset'))
