...
void* GetProcAddress(void* hmod, const char* sym) { return dlsym(hmod, sym); }
...
const char* AppFramework::GetLastError() { return g_last_error; }

void AppFramework::SetLoadLibrary(std::function<void*(const char*)> fn) { g_load_library = fn; }

void AppFramework::SetFreeLibrary(std::function<void(void*)> fn) { g_free_library = fn; }
...
void* _LoadLibraryA(const char* lib) { return g_load_library(lib); }

void _FreeLibrary(void* lib) { g_free_library(lib); }
...
bool AppFramework::AddInterface(const char* module, const char* iface)
{
	...
	{
		Q_snprintf(g_last_error, sizeof(g_last_error), "Number of interfaces is 0, or iface_list is nullptr in module %s while loading %s",
			   module, iface);
		return false;
	}
...
bool AppFramework::AddInterfaces(std::initializer_list<interface_t> interfaces)
{
	...
	{
		if (!AppFramework::AddInterface(x.module, x.iface))
			return false;
	}
	return true;
}
...
bool AppFramework::AddInterfaces(interface_t* interfaces)
{
	...
	{
		if (!AppFramework::AddInterface(interfaces[i].module, interfaces[i].iface))
			return false;
	}
	return true;
}
