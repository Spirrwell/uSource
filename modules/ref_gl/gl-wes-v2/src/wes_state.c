...
static wrapState wrapglInitState = {GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE, GL_TRUE, GL_FALSE,
				    GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE};

GLvoid wes_state_update(void)
{
...
const char* wes_name_envsrc(GLint param)
{
	switch (param)
	{
	case GL_PREVIOUS:
		return "WES_SRC_PREVIOUS";
	case GL_CONSTANT:
		return "WES_SRC_CONSTANT";
	case GL_PRIMARY_COLOR:
		return "WES_SRC_PRIMARY_COLOR";
	case GL_TEXTURE:
		return "WES_SRC_TEXTURE";

	/* Crossbar Extensions */
	case GL_TEXTURE0:
		return "WES_SRC_TEXTURE0";
	case GL_TEXTURE1:
		return "WES_SRC_TEXTURE1";
	case GL_TEXTURE2:
		return "WES_SRC_TEXTURE2";
	case GL_TEXTURE3:
		return "WES_SRC_TEXTURE3";
...
const char* wes_name_envfunc(GLint param)
{
	switch (param)
	{
	case GL_REPLACE:
		return "WES_FUNC_REPLACE";
	case GL_MODULATE:
		return "WES_FUNC_MODULATE";
	case GL_ADD:
		return "WES_FUNC_ADD";
	case GL_DECAL:
		return "WES_FUNC_DECAL";
	case GL_BLEND:
		return "WES_FUNC_BLEND";
	case GL_COMBINE:
		return "WES_FUNC_COMBINE";
	case GL_ADD_SIGNED:
		return "WES_FUNC_ADD_SIGNED";
	case GL_INTERPOLATE:
		return "WES_FUNC_INTERPOLATE";
	case GL_SUBTRACT:
		return "WES_FUNC_SUBTRACT";
	case GL_DOT3_RGB:
		return "WES_FUNC_DOT3_RGB";
	case GL_DOT3_RGBA:
		return "WES_FUNC_DOT3_RGBA";
#if 0
        /* ATI Extensions */
...
GLint wes_index_envfunc(GLint param)
{
	switch (param)
	{
	case GL_REPLACE:
...
GLint wes_index_envsrc(GLint param)
{
	switch (param)
	{
	case GL_PREVIOUS:
...
GLint wes_index_envop(GLint param)
{
	switch (param)
	{
	case GL_SRC_ALPHA:
	...
	case GL_ONE_MINUS_SRC_ALPHA:
		return WES_OP_ONE_MINUS_ALPHA;
	case GL_SRC_COLOR:
		return WES_OP_COLOR;
	case GL_ONE_MINUS_SRC_COLOR:
		return WES_OP_ONE_MINUS_COLOR;
	default:
		return 0;
	}
}

GLvoid wes_setstate_old(GLenum e, GLboolean b)
{
	...
	case GL_TEXTURE_GEN_Q:
		...
		break;
	case GL_CLIP_PLANE0:
		SetUniform1i(uEnableClipPlane[0], b);
		break;
	case GL_CLIP_PLANE1:
		SetUniform1i(uEnableClipPlane[1], b);
		break;
	case GL_CLIP_PLANE2:
		SetUniform1i(uEnableClipPlane[2], b);
		break;
	case GL_CLIP_PLANE3:
		SetUniform1i(uEnableClipPlane[3], b);
		break;
	case GL_CLIP_PLANE4:
		SetUniform1i(uEnableClipPlane[4], b);
		break;
	case GL_CLIP_PLANE5:
		SetUniform1i(uEnableClipPlane[5], b);
		break;

	case GL_LIGHTING:
		SetUniform1i(uEnableLighting, b);
		break;
	case GL_LIGHT0:
		SetUniform1i(uEnableLight[0], b);
		break;
	case GL_LIGHT1:
		SetUniform1i(uEnableLight[1], b);
		break;
	case GL_LIGHT2:
		SetUniform1i(uEnableLight[2], b);
		break;
	case GL_LIGHT3:
		SetUniform1i(uEnableLight[3], b);
		break;
	case GL_LIGHT4:
		SetUniform1i(uEnableLight[4], b);
		break;
	case GL_LIGHT5:
		SetUniform1i(uEnableLight[5], b);
		break;
	case GL_LIGHT6:
		SetUniform1i(uEnableLight[6], b);
		break;
	case GL_LIGHT7:
		SetUniform1i(uEnableLight[7], b);
		break;
...
GLboolean wes_getstate(GLenum e)
{
	...
	{
	case GL_ALPHA_TEST:
		return wrapglState.alpha_test;
	case GL_BLEND:
		return wrapglState.blend;
	case GL_COLOR_MATERIAL:
		return wrapglState.color_material;
	case GL_CULL_FACE:
		return wrapglState.cull_face;
	case GL_DEPTH_TEST:
		return wrapglState.depth_test;
	case GL_DITHER:
		return wrapglState.dither;
	case GL_FOG:
		return u_progstate.uEnableFog; // UNSUPPORTED?
	case GL_LIGHTING:
		return wrapglState.lighting;
	case GL_NORMALIZE:
		return wrapglState.normalize;
	case GL_POLYGON_OFFSET_FILL:
		return wrapglState.polygon_offset_fill;
	case GL_RESCALE_NORMAL:
		return wrapglState.rescale_normal;
	case GL_SAMPLE_ALPHA_TO_COVERAGE:
		return wrapglState.sample_alpha_to_coverage;
	case GL_SAMPLE_COVERAGE:
		return wrapglState.sample_coverage;
	case GL_SCISSOR_TEST:
		return wrapglState.scissor_test;
	case GL_STENCIL_TEST:
		return GL_FALSE; // UNSUPPORTED!
	case GL_TEXTURE_2D:
		return u_progstate.uTexture[u_activetex].Enable;
	default:
...
GLvoid GL_MANGLE(glLightf)(GLenum light, GLenum pname, GLfloat params)
{
...
GLvoid GL_MANGLE(glLightfv)(GLenum light, GLenum pname, GLfloat* params)
{
	GLuint ind = light - GL_LIGHT0;
	wes_vertbuffer_flush();
...
GLvoid GL_MANGLE(glMaterialf)(GLenum face, GLenum pname, GLfloat params)
{
	wes_vertbuffer_flush();
...
GLvoid GL_MANGLE(glMaterialfv)(GLenum face, GLenum pname, GLfloat* params)
{
	wes_vertbuffer_flush();
...
GLvoid GL_MANGLE(glLightModelfv)(GLenum pname, GLfloat* params)
{
	wes_vertbuffer_flush();
...
GLvoid GL_MANGLE(glColorMaterial)(GLenum face, GLenum mode)
{
...
GLvoid GL_MANGLE(glEnable)(GLenum e) { wes_setstate(e, GL_TRUE); }

GLvoid GL_MANGLE(glDisable)(GLenum e) { wes_setstate(e, GL_FALSE); }

GLboolean GL_MANGLE(glIsEnabled)(GLenum e) { return wes_getstate(e); }

GLvoid GL_MANGLE(glAlphaFunc)(GLenum func, GLclampf ref)
{
	wes_vertbuffer_flush();
...
GLvoid GL_MANGLE(glFogi)(GLenum pname, GLint param)
{
	wes_vertbuffer_flush();
...
GLvoid GL_MANGLE(glFogf)(GLenum pname, GLfloat param)
{
	wes_vertbuffer_flush();
...
GLvoid GL_MANGLE(glFogfv)(GLenum pname, GLfloat* param)
{
	wes_vertbuffer_flush();
...
GLvoid GL_MANGLE(glTexGeni)(GLenum coord, GLenum pname, GLint param)
{
...
GLvoid GL_MANGLE(glTexGenfv)(GLenum coord, GLenum pname, GLfloat* param)
{
	wes_vertbuffer_flush();
...
GLvoid GL_MANGLE(glActiveTexture)(GLenum texture)
{
	// wes_vertbuffer_flush(); ?
...
GLvoid GL_MANGLE(glTexEnvi)(GLenum target, GLenum pname, GLint param)
{
...
GLvoid GL_MANGLE(glTexEnvf)(GLenum target, GLenum pname, GLfloat param) { GL_MANGLE(glTexEnvi)(target, pname, param); }
...
GLvoid GL_MANGLE(glClipPlane)(GLenum plane, const GLdouble* equation)
{
	GLint ind = plane - GL_CLIP_PLANE0;
	wes_vertbuffer_flush();

	SetUniform4f(uClipPlane[ind], (GLfloat)equation[0], (GLfloat)equation[1], (GLfloat)equation[2], (GLfloat)equation[3]);
}
