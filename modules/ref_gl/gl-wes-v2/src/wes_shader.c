...
char*	  wesShaderFallbackStr = "attribute highp vec4 	aPosition;\n\
	attribute lowp vec4 	aColor;\n\
	attribute mediump vec4 	aTexCoord0;\n\
	attribute mediump vec4 	aTexCoord1;\n\
	attribute mediump vec4 	aTexCoord2;\n\
...
GLvoid wes_shader_error(GLuint ind)
{
	int   len;
	char* log;
	int   i;
...
GLvoid wes_program_error(GLuint ind)
{
	int   len;
	char* log;
	int   i;
...
GLuint wes_shader_create(char* data, GLenum type)
{
	GLuint index;
	GLint  success;
...
GLvoid wes_attrib_loc(GLuint prog)
{
...
GLvoid wes_uniform_loc(program_t* p)
{
#define LocateUniform(A) p->uloc.A = wes_gl->glGetUniformLocation(p->prog, #A);
...
GLuint wes_program_create(GLuint frag, GLuint vert)
{
	GLuint prog;
	GLint  success;
...
GLvoid wes_build_program(progstate_t* s, program_t* p)
{
	char frag[4096];
	memset(frag, 0, 4096);
...
GLboolean wes_progstate_cmp(progstate_t* s0, progstate_t* s1)
{
	GLint i, j;
...
GLvoid wes_bind_program(program_t* p)
{
	if (p->isbound)
		return;
	if (sh_program)
...
GLvoid wes_choose_program(progstate_t* s)
{
	unsigned int i, j;
	program_t*   p;
			...
			if (sh_program != &sh_pbuffer[i])
			{
				...
				for(j = 0; j < WES_MULTITEX_NUM; j++){
					if (s->uTexture[j].Enable){
						...
						fprintf(stdout, "TEX%i RGB COMB %i ALPHA COMB %i \n", j, s->uTexture[j].RGBCombine,
				s->uTexture[j].AlphaCombine); fprintf(stdout, "TEX%i RGB SRC = [%i, %i, %i] \n", j, s->uTexture[j].Arg[0].RGBSrc,
				s->uTexture[j].Arg[1].RGBSrc,s->uTexture[j].Arg[2].RGBSrc); fprintf(stdout, "TEX%i ALPHA SRC = [%i, %i, %i] \n", j,
				s->uTexture[j].Arg[0].AlphaSrc, s->uTexture[j].Arg[1].AlphaSrc,s->uTexture[j].Arg[2].AlphaSrc); fprintf(stdout, "TEX%i
				RGB OP = [%i, %i, %i] \n", j, s->uTexture[j].Arg[0].RGBOp, s->uTexture[j].Arg[1].RGBOp,s->uTexture[j].Arg[2].RGBOp);
						fprintf(stdout, "TEX%i ALPHA OP = [%i, %i, %i] \n", j, s->uTexture[j].Arg[0].AlphaOp,
				s->uTexture[j].Arg[1].AlphaOp,s->uTexture[j].Arg[2].AlphaOp);
					}
				}*/
			}
			return;
...
GLvoid wes_shader_destroy(void)
{
	unsigned int i;
