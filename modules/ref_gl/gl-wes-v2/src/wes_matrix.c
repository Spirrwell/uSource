...
/* internal functions   */
GLvoid wes_assign(GLfloat* in, matrix4_t* out)
{
...
GLvoid wes_classify(matrix4_t* in)
{
	in->flags = 0;
	...
	if (in->data[1] == 0 && in->data[2] == 0 && in->data[4] == 0 && in->data[6] == 0 && in->data[8] == 0 && in->data[9] == 0)
	{
...
GLvoid wes_transpose2(GLfloat* in, GLfloat* out)
{
	out[0] = in[0];
	out[1] = in[2];
	out[2] = in[1];
...
GLvoid wes_transpose3(GLfloat* in, GLfloat* out)
{
	out[0] = in[0];
	out[1] = in[3];
	out[2] = in[6];
...
GLvoid wes_transpose4(GLfloat* in, GLfloat* out)
{
	out[0]	= in[0];
	out[1]	= in[4];
	out[2]	= in[8];
...
GLvoid cross3(GLfloat* v1, GLfloat* v2, GLfloat* result)
{
...
GLvoid wes_mul4_general(GLfloat* in0, GLfloat* in1, GLfloat* out)
{
...
GLvoid wes_identity4(matrix4_t* m)
{
...
GLvoid wes_mat4to3(matrix4_t* in, matrix3_t* out)
{
...
GLvoid wes_mul4(matrix4_t* in0, matrix4_t* in1, matrix4_t* out)
{
...
GLvoid wes_rotate_xyz(GLfloat angle, GLfloat x, GLfloat y, GLfloat z)
{
	GLfloat mag;
...
GLvoid wes_rotate_z(GLfloat angle, GLfloat z)
{
	GLfloat s, c;
	GLfloat m1, m4;
	GLfloat tmp[8];
...
GLvoid wes_rotate_y(GLfloat angle, GLfloat y)
{
	GLfloat s, c;
	GLfloat m2, m8;
	GLfloat tmp[8];
...
GLvoid wes_rotate_x(GLfloat angle, GLfloat x)
{
	GLfloat s, c;
	GLfloat m6, m9;
	GLfloat tmp[8];
...
GLvoid wes_normal_matrix(matrix4_t* in, matrix3_t* out)
{
...
GLvoid wes_matvec4(matrix4_t* m, GLfloat* v, GLfloat* mv)
{
...
GLvoid wes_matrix_fprintf(FILE* f, matrix4_t* in)
{
	int i;
...
GLboolean wes_matrix_normal(void)
{
	if (m_modelview_mod)
	{
...
GLvoid wes_matrix_update(void) { m_modelview_mod = m_projection_mod = 0; }
...
GLvoid GL_MANGLE(glMatrixMode)(GLenum mode)
{
	...
	case GL_MODELVIEW:
		...
		break;
	case GL_PROJECTION:
		m_current = m_projection;
		break;
	case GL_TEXTURE:
		m_current = &m_texture[u_activetex * WES_TEXTURE_NUM];
		break;
	case GL_COLOR:
		m_current = m_color;
		break;
...
GLvoid GL_MANGLE(glLoadMatrixf)(GLfloat* m)
{
	wes_vertbuffer_flush();
...
GLvoid GL_MANGLE(glMultMatrixf)(GLfloat* m)
{
...
GLvoid GL_MANGLE(glLoadIdentity)(void)
{
	wes_vertbuffer_flush();
...
GLvoid GL_MANGLE(glRotatef)(GLfloat angle, GLfloat x, GLfloat y, GLfloat z)
{
	wes_vertbuffer_flush();
...
GLvoid GL_MANGLE(glTranslatef)(GLfloat x, GLfloat y, GLfloat z)
{
	wes_vertbuffer_flush();
...
GLvoid GL_MANGLE(glScalef)(GLfloat x, GLfloat y, GLfloat z)
{
	wes_vertbuffer_flush();
...
GLvoid GL_MANGLE(glFrustrumf)(float l, float r, float b, float t, float n, float f)
{
...
GLvoid GL_MANGLE(glPushMatrix)(void)
{
	unsigned int i;
	wes_vertbuffer_flush();
...
GLvoid GL_MANGLE(glPopMatrix)(void)
{
	unsigned int i;
	wes_vertbuffer_flush();
...
GLvoid gluPerspective(GLfloat fovy, GLfloat aspect, GLfloat znear, GLfloat zfar)
{
...
GLvoid gluLookAt(GLfloat eyex, GLfloat eyey, GLfloat eyez, GLfloat centerx, GLfloat centery, GLfloat centerz, GLfloat upx, GLfloat upy, GLfloat upz)
{
