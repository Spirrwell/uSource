...
static void  dlclose(void* hInstance) { FreeLibrary((HMODULE)hInstance); }
static void* dlsym(void* hInstance, const char* name) { return GetProcAddress((HMODULE)hInstance, name); }
#else
...
#endif
GLvoid wes_init(const char* gles2)
{
	int    i;
	void** ptr;
...
GLvoid GL_MANGLE(glScissor)(GLint x, GLint y, GLsizei width, GLsizei height) { wes_gl->glScissor(x, y, width, height); }
