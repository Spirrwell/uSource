...
GLint wes_frag_clipplane(char* buff, progstate_t* s)
{
	char* str = buff;
...
GLint wes_frag_fog(char* buff, progstate_t* s)
{
	char* str = buff;
	if (s->uEnableFog)
	{
...
GLint wes_frag_alphatest(char* buff, progstate_t* s)
{
	char* str = buff;
...
GLint wes_frag_op(char* buff, progstate_t* s, int tex, int arg)
{
	char* str = buff;
...
GLint wes_frag_arg_obtain(char* buff, progstate_t* s, int tex, int arg, int type)
{
	char* str = buff;
	...
	case 1:
		...
		break;
	default:
		PRINT_ERROR("Unknown Argument Type: %i", type);
		break;
	}
	...
	default:
		PRINT_ERROR("ERROR: No valid SRC for TEX%i ARG%i.%s", tex, arg, comp);
		break;
	}
	return (GLint)(str - buff);
};

GLvoid wes_frag_nargs(progstate_t* s, GLint tex, GLint* narg_rgb, GLint* narg_alpha)
{
	*narg_rgb = *narg_alpha = 0;
	...
	default:
		PRINT_ERROR("Could not determine number of neccessary arguments for Texture Unit %i", tex);
	}
...
GLint wes_frag_args(char* buff, progstate_t* s, int tex)
{
	char* str = buff;
...
GLint wes_frag_combine(char* buff, progstate_t* s, int tex)
{
	char* str = buff;
	...
	if (s->uTexture[tex].RGBCombine == s->uTexture[tex].AlphaCombine)
	{
		...
		default:
			PRINT_ERROR("ERROR: No valid FUNC for TEX%i", tex);
			break;
		}
	}
	...
	else
	{
		...
		default:
			PRINT_ERROR("ERROR: No valid FUNC for TEX%i.rgb", tex);
			break;
		}
		...
		default:
			PRINT_ERROR("ERROR: No valid FUNC for TEX%i.a", tex);
			break;
		}
	}
...
GLint wes_frag_tex_obtain(char* buff, progstate_t* s)
{
	char* str = buff;
...
GLint wes_frag_tex(char* buff, progstate_t* s)
{
	char* str = buff;
	GLint i;
	...
	for (i = 0; i < WES_MULTITEX_NUM; i++)
	{
		if (s->uTexture[i].Enable)
		{
			...
			default:
				PRINT_ERROR("ERROR: No valid MODE for TEX%i", i);
				break;
			}
		}
	}
...
GLvoid wes_frag_build(char* buff, progstate_t* s)
{
	char* str = buff;
	int   i;
	...
	for (i = 0; i < WES_MULTITEX_NUM; i++)
	{
		if (s->uTexture[i].Enable)
		{
			...
			fprintf(stdout, "TEX%i RGB SRC = [%i, %i, %i] \n", i, s->uTexture[i].Arg[0].RGBSrc, s->uTexture[i].Arg[1].RGBSrc,
				s->uTexture[i].Arg[2].RGBSrc);
			fprintf(stdout, "TEX%i ALPHA SRC = [%i, %i, %i] \n", i, s->uTexture[i].Arg[0].AlphaSrc, s->uTexture[i].Arg[1].AlphaSrc,
				s->uTexture[i].Arg[2].AlphaSrc);
			fprintf(stdout, "TEX%i RGB OP = [%i, %i, %i] \n", i, s->uTexture[i].Arg[0].RGBOp, s->uTexture[i].Arg[1].RGBOp,
				s->uTexture[i].Arg[2].RGBOp);
			fprintf(stdout, "TEX%i ALPHA OP = [%i, %i, %i] \n", i, s->uTexture[i].Arg[0].AlphaOp, s->uTexture[i].Arg[1].AlphaOp,
				s->uTexture[i].Arg[2].AlphaOp);
		}
	}
