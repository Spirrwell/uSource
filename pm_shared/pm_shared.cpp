...
qboolean PM_InWater(void) { return (pmove->waterlevel > 1); }
...
void PM_ResetStuckOffsets(int nIndex, int server) { rgStuckLast[nIndex][server] = 0; }
...
void PM_Jump(void)
{
	...
	if ((pmove->bInDuck) || (pmove->flags & FL_DUCKING))
	{
		...
		if (cansuperjump && (pmove->cmd.buttons & IN_DUCK) && (pmove->flDuckTime > 0) && VectorLength(pmove->velocity) > 50)
		{
